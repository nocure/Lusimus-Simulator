...
static void arm_reset_state() {
	...
	if (preset_regfile[1]) state->Reg[1] = preset_regfile[1];

}
static void arm_init_state() {
	ARMul_EmulateInit();
...
//      skyeye_option_init (&skyeye_config);
	...
	// ww: original value was 50, changed to 1 to increase speed of io polling and timer
	skyeye_config.mach->io_cycle_divisor = 1;
	skyeye_config.mach->mach_init(state, skyeye_config.mach);
...
static void arm_set_pc(WORD pc) {
	state->Reg[15] = pc;
}
static WORD arm_get_pc() {
	return (WORD)state->Reg[15];
}
static int arm_ICE_write_byte(WORD addr, uint8_t v) {
...
//	int ret;
	...
	return bus_read(8, addr, (uint32_t*)pv);
}
...
cpu_config_t arm_cpus[] = { { "armv3", "arm710", 0x41007100, 0xfff8ff00, DATACACHE }, { "armv3", "arm7tdmi", 0x41007700, 0xfff8ff00, NONCACHE }, { "armv4",
		"arm720t", 0x41807200, 0xffffff00, DATACACHE }, { "armv4", "sa1110", SA1110, 0xfffffff0, INSTCACHE },
		{ "armv4", "sa1100", SA1100, 0xffffffe0, INSTCACHE }, { "armv4", "arm920t", 0x41009200, 0xff00fff0, INSTCACHE }, { "armv5", "arm926ejs", 0x41069260,
				0xff0ffff0, INSTCACHE }, { "xscale", "pxa25x", PXA250, 0xfffffff0, INSTCACHE }, { "xscale", "pxa27x", PXA270, 0xfffffff0, INSTCACHE }, {
				"armv6", "arm11", 0, 0x0, 0 } };

static int arm_parse_cpu(const char *params[]) {
	int i;
	for (i = 0; i < (sizeof(arm_cpus) / sizeof(cpu_config_t)); i++) {
		if (!strncmp(params[0], arm_cpus[i].cpu_name, MAX_PARAM_NAME)) {

			p_arm_cpu = &arm_cpus[i];
			SKYEYE_INFO("cpu info: %s, %s, %x, %x, %x \n", p_arm_cpu->cpu_arch_name, p_arm_cpu->cpu_name, p_arm_cpu->cpu_val, p_arm_cpu->cpu_mask, p_arm_cpu->cachetype);
...
static int arm_parse_mach(machine_config_t * mach, const char *params[]) {
	int i;
	for (i = 0; i < (sizeof(arm_machines) / sizeof(machine_config_t)); i++) {
		if (!strncmp(params[0], arm_machines[i].machine_name,
		MAX_PARAM_NAME)) {
...
extern void flash_write_byte(ARMul_State * state, ARMword addr, ARMword data);
extern ARMword flash_read_halfword(ARMul_State * state, ARMword addr);
extern void flash_write_halfword(ARMul_State * state, ARMword addr, ARMword data);
extern ARMword flash_read_word(ARMul_State * state, ARMword addr);
extern void flash_write_word(ARMul_State * state, ARMword addr, ARMword data);
#if 0
static int
...
// ww 2018-0817 this regfile stuff is far far away from complete.
// conf: regfile: r1=0x100, r2=0x200, ...
static int arm_parse_regfile(int num_params, const char *params[]) {
	char name[MAX_PARAM_NAME], value[MAX_PARAM_NAME];
	int i;
	int reg_value;
	for (i = 0; i < num_params; i++) {
		if (split_param(params[i], name, value) < 0) {
			SKYEYE_ERR("Error: unknown parameter \"%s\".\n", params[i]);
		}
			...
			SKYEYE_ERR("Error: unsupported parameter \"%s\".\n", name);
		}
	}
	return 0;
}
