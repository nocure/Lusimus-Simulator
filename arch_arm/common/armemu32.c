...
int ARMul_ICE_debug(ARMul_State *state, ARMword instr, ARMword addr) {
	...
	if (debugmode) {
		...
		if (skyeye_ice.tps_status == TRACE_STARTED) {
			for (i = 0; i < skyeye_ice.num_tps; i++) {
				if (((skyeye_ice.tps[i].tp_address == addr) && (skyeye_ice.tps[i].status == TRACEPOINT_ENABLED))
						|| (skyeye_ice.tps[i].status == TRACEPOINT_STEPPING)) {
					handle_tracepoint(i);
				}
			}
		}
	...
	if (skyeye_config.code_cov.prof_on) cov_prof(EXEC_FLAG, addr);
#endif
	return 0;
}
/*
		...
		if (skyeye_config.log.logon >= 1) {
			if (state->NumInstrs >= skyeye_config.log.start && state->NumInstrs <= skyeye_config.log.end) {
				...
				if (pc == 0xc00261ac) {
					//sleep(2);
					mybeg3 = 1;
					printf ("************after tlb-flush  numinstr is %I64u  ****************\n", state->NumInstrs);
				}
				if (mybeg3 == 1) {
				...
				if (mybegin == 1) {
					...
					/*if(state->NumInstrs>=3302294) {
					 ...
					 chy_debug();
					 printf("*********************************************\n");
					 printf("****** N %I64x :p %x,i %x\n  ******\n",state->NumInstrs,pc,instr);
					 printf("*********************************************\n");
					 }
					 */
					if (skyeye_config.log.logon >= 1) fprintf(skyeye_logfd, "N %I64x :p %x,i %x,", state->NumInstrs, pc,
#ifdef MODET
		...
		if (temp) {
				...
				if (BIT (20) == 0 && BITS (25, 27) == 0) {
					if (BITS (4, 7) == 0xD) {
						...
						if (BIT(12))
							...
							state->Reg[BITS(12, 15)] = ARMul_LoadWordN(state, addr);
							state->Reg[BITS
							(12,
									15) + 1] = ARMul_LoadWordN(state, addr + 4);
							if (wb)
							LSBase= temp2;
						}
						...
						if (BIT(12))
							...
							ARMul_StoreWordN(state, addr, state->Reg[BITS(12, 15)]);
							ARMul_StoreWordN(state, addr + 4, state->Reg[BITS
							(12,
									15) + 1]);
			...
			case 0x08: /* ADD reg */
				...
				if (BITS (4, 7) == 0x9) {
					/* MULL */
					/* 32x32 = 64 */
					ARMul_Icycles(state, Multiply64(state, instr,
					LUNSIGNED,
					LDEFAULT), 0L);
					break;
				}
			...
			case 0x0a: /* ADC reg */
				...
				if (BITS (4, 7) == 0x9) {
					/* MULL */
					/* 32x32=64 */
					ARMul_Icycles(state, MultiplyAdd64(state, instr,
					LUNSIGNED,
					LDEFAULT), 0L);
					break;
				}
			...
			case 0x10: /* TST reg and MRS CPSR and SWP word.  */
				if (state->is_v5e) {
					if (BIT (4) == 0 && BIT (7) == 1) {
						...
						op1 &= 0xFFFF;
						op2 &= 0xFFFF;
						if (op1 & 0x8000) op1 -= 65536;
						if (op2 & 0x8000) op2 -= 65536;
						op1 *= op2;
...
//						ARMword value;
						...
						if (!SWI_vector_installed)
							...
							ARMul_CP15_R5_MMU_EXCPT, pc);
							if (!XScale_debug_moe(state,
							ARMul_CP14_R10_MOE_BT)) break;
						}
			...
			case 0x16: /* CMN reg and MSR reg to SPSR */
				...
				if (state->is_v5) {
					if (BITS (4, 11) == 0xF1 && BITS (16, 19) == 0xF) {
						...
						if (op1) for (result = 0; (op1 & 0x80000000) == 0; op1 <<= 1)
							result++;
...
//chy 2006-04-12 for ICE debug
		TEST_EMULATE: if (state->Emulate == ONCE)
			state->Emulate = STOP;
...
ARMul_Emulate32_dbct (ARMul_State * state)
{
	...
	do {
		/*if(skyeye_config.log.logon>=1){
		 ...
		 if(mybegin==1) {
		 state->Reg[15] -= INSN_SIZE;
		 if(skyeye_config.log.logon>=1) fprintf(skyeye_logfd,"N %I64x :p %x,i %x,",state->NumInstrs, (state->Reg[15] - INSN_SIZE), instr);
		...
		if (!gen_func) {
			//fprintf(stderr, "tb_find: Error in find the translate block.\n");
			//exit(-1);
			//TRAP_INSN_ABORT
			//TEA_OUT(printf("\n------------\npc:%x\n", state->Reg[15] - INSN_SIZE));
	...
	if (BIT(4)) {
		...
		case ASR:
			if (shamt == 0)
				...
				ASSIGNC((ARMword )
				((int ) base >> (int ) (shamt - 1)) & 1);
				return ((ARMword)((int)base >> (int)shamt));
			}
		case ROR:
			if (shamt == 0) return (base);
			shamt &= 0x1f;
			if (shamt == 0) {
				ASSIGNC(base >> 31);
				return (base);
...
#define MEM_LOAD_LOG(description) if (skyeye_config.log.memlogon >= 1) { \
		fprintf(skyeye_logfd, \
			"m LOAD %s: N %I64x :p %x :i %x :a %x :d %x\n", \
...
#define MEM_STORE_LOG(description) if (skyeye_config.log.memlogon >= 1) { \
		fprintf(skyeye_logfd, \
			"m STORE %s: N %I64x :p %x :i %x :a %x :d %x\n", \
			description, state->NumInstrs, state->pc, instr, \
			address, DEST); \
	}
...
static unsigned LoadHalfWord(ARMul_State * state, ARMword instr, ARMword address, int signextend) {
	...
	if (signextend) if (dest & 1 << (16 - 1)) dest = (dest & ((1 << 16) - 1)) - (1 << 16);
...
static unsigned LoadByte(ARMul_State * state, ARMword instr, ARMword address, int signextend) {
	...
	if (signextend) if (dest & 1 << (8 - 1)) dest = (dest & ((1 << 8) - 1)) - (1 << 8);
...
static void Handle_Load_Double(ARMul_State * state, ARMword instr) {
	...
	if ((!pre_indexed || write_back) && (addr_reg == dest_reg || addr_reg == dest_reg + 1)) {
		ARMul_UndefInstr(state, instr);
		return;
	}
	...
	if (!pre_indexed) addr = sum;

	if (!pre_indexed || write_back) state->Reg[addr_reg] = addr;
}
...
static void Handle_Store_Double(ARMul_State * state, ARMword instr) {
	...
	/* For pre indexed or post indexed addressing modes,
	 check that the destination registers do not overlap
	 the address registers.  */
	if ((!pre_indexed || write_back) && (addr_reg == src_reg || addr_reg == src_reg + 1)) {
		ARMul_UndefInstr(state, instr);
		return;
	}
...
static void LoadMult(ARMul_State * state, ARMword instr, ARMword address, ARMword WBBase) {
	...
	UNDEF_LSMNoRegs; UNDEF_LSMPCBase; UNDEF_LSMBaseInListWb;
	BUSUSEDINCPCS;
#ifndef MODE32
	if (ADDREXCEPT (address))
	INTERNALABORT (address);
	...
	if (state->Aborted) goto L_ldm_makeabort;
	...
	for (; temp < 16; temp++)
		if (BIT(temp)) {
			...
			if (!state->abortSig && !state->Aborted)
				...
				XScale_set_fsr_far(state,
				ARMul_CP15_R5_ST_ALIGN, address);
				state->Aborted = ARMul_DataAbortV;
			}
			/*chy 2004-05-23 chy goto end */
			if (state->Aborted) goto L_ldm_makeabort;
...
static void LoadSMult(ARMul_State * state, ARMword instr, ARMword address, ARMword WBBase) {
	ARMword dest, temp;
	...
	/*chy 2004-05-23 chy goto end*/
	if (state->Aborted) goto L_ldm_s_makeabort;
	/* S cycles from here on.  */
	for (; temp < 16; temp++)
		if (BIT(temp)) {
			/* Load this register.  */
	...
	/*chy 2004-05-23 should compare the Abort Models*/
	if (state->Aborted) {
		if (BIT (21) && LHSReg != 15) if (!(state->abortSig && state->Aborted && state->lateabtSig == LOW))
		LSBase= WBBase;
		TAKEABORT;
	}
	else if (BIT (21) && LHSReg != 15)
...
static void StoreMult(ARMul_State * state, ARMword instr, ARMword address, ARMword WBBase) {
	ARMword temp;
...
//chy 2004-05-23,should compare the Abort Models
	L_stm_takeabort: if (BIT (21) && LHSReg != 15) {
		if (!(state->abortSig && state->Aborted && state->lateabtSig == LOW))
		LSBase= WBBase;
	}
	if (state->Aborted)
...
static void StoreSMult(ARMul_State * state, ARMword instr, ARMword address, ARMword WBBase) {
	ARMword temp;
...
//chy 2004-05-23, needn't store other when aborted
	if (state->Aborted) goto L_stm_s_takeabort;
	/* S cycles from here on.  */
	for (; temp < 16; temp++)
		if (BIT(temp)) {
			/* Save this register.  */
...
//chy 2004-05-23,should compare the Abort Models
	L_stm_s_takeabort: if (BIT (21) && LHSReg != 15) {
		if (!(state->abortSig && state->Aborted && state->lateabtSig == LOW))
		LSBase= WBBase;
	}
...
static unsigned Multiply64(ARMul_State * state, ARMword instr, int msigned, int scc) {
	...
	if (nRdHi != 15 && nRdLo != 15 && nRs != 15 && nRm != 15 && nRdHi != nRdLo && nRdHi != nRm && nRdLo != nRm) {
		...
		if (msigned) {
			...
			if (((signed int)Rm) < 0) Rm = -Rm;

			if (((signed int)Rs) < 0) Rs = -Rs;
		}
