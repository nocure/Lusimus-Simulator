...
*************************************************/
float Calculate_Mutilplier(ARMul_State * state) {
...
//      printf("L=%d,M=%d,N=%I64f,Core_freq=%f",L,M,N,Core_freq);

// ww 2018-0817 modified -->
//even though freq & vdd won't matter in a simulator,  I really admire the guy.
#if 0
	if (Core_freq < 100) {
		Vdd = 0.85;
	}
...
void ARMul_EnergyInit(ARMul_State * state) {
...
void ARMul_do_energy(ARMul_State * state, ARMword instr, ARMword pc) {
	int opcode;
	...
	if (state->energy.energy_prof) {	// BUG200103282109
		state->energy.tcycle = state->NumScycles + state->NumNcycles + state->NumIcycles;
		ex_cycle = state->energy.tcycle - state->energy.pcycle;
		mem_cycle = state->energy.t_mem_cycle - state->energy.p_mem_cycle;
		idle_cycle = state->energy.t_idle_cycle - state->energy.p_idle_cycle;
		uart_cycle = state->energy.t_uart_cycle - state->energy.p_uart_cycle;
		...
		cur_cyc_energy = ex_energy + mem_energy + idle_energy + uart_energy + mem_idle_energy;
