...
void mem_reset(ARMul_State * state) {
	...
	for (i = 0; i < num; i++) {
		bank = i;
		if (state->mem.rom[bank]) free(state->mem.rom[bank]);
		...
		if (mb[bank].type != MEMTYPE_IO) {
			...
			if (!state->mem.rom[bank]) {
				fprintf(stderr, "mem_reset: Error allocating mem for bank number %d.\n", bank);
				skyeye_exit(-1);
			}
			/*ywc 2005-04-01 */
#ifdef DBCT
			if (!skyeye_config.no_dbct) {
				...
				if (mb[bank].len % TB_LEN) {
					fprintf (stderr,
							"mem_reset: Bank number %d length error.\n",
							bank);
					skyeye_exit (-1);
				}
...
#endif
			if (mb[bank].filename && (f = fopen(mb[bank].filename, "rb"))) {
				...
				while (s < state->mem.rom_size[bank]) {
					if (state->bigendSig == HIGH) /*big enddian? */
						swap = ((ARMword)p[3]) | (((ARMword)p[2]) << 8) | (((ARMword)p[1]) << 16) | (((ARMword)p[0]) << 24);
					else
				...
				fprintf(stderr, "bank %d, Couldn't open boot ROM %s - execution will "
						"commence with the debuger.\n", bank, mb[bank].filename);
				skyeye_exit(-1);
			}
		}
	} /*end  for(i = 0;i < num; i++) */
...
ARMword mem_read_byte(ARMul_State * state, ARMword addr) {
	...
	if (global_mbp && global_mbp->read_byte)
		...
		//fprintf(stderr, "NumInstrs %I64u, mem_read_byte addr = %x no bank\n",state->NumInstrs, addr);
		//chy 2003-09-03
...
ARMword mem_read_halfword(ARMul_State * state, ARMword addr) {
	...
	if (global_mbp && global_mbp->read_halfword)
		...
		//fprintf(stderr, "NumInstrs %I64u, mem_read_halfword addr = %x no bank\n",state->NumInstrs, addr);
		//chy 2003-09-03
		//SKYEYE_OUTREGS(stderr);
		//exit(-1);
		return 0;
...
ARMword mem_read_word(ARMul_State * state, ARMword addr) {
	if (skyeye_config.code_cov.prof_on) cov_prof(READ_FLAG, addr);
	...
	if (global_mbp && global_mbp->read_word)
		...
		fprintf(stderr, "Error in %s, no bank found, NumInstrs %I64u, mem_read_word addr = %x no bank\n", __FUNCTION__, state->NumInstrs, addr);
		//SKYEYE_OUTREGS(stderr);
		//skyeye_exit(-1);
		return 0;
	}
...
void mem_write_byte(ARMul_State * state, ARMword addr, ARMword data) {
	...
	if (global_mbp && global_mbp->write_byte) {
		...
		fprintf(stderr, "NumInstrs %I64u, mem_write_byte addr = %x no bank\n", state->NumInstrs, addr);
		//chy 2003-09-03
		//SKYEYE_OUTREGS(stderr);
		skyeye_exit(-1);
	}
...
void mem_write_halfword(ARMul_State * state, ARMword addr, ARMword data) {
	...
	if (global_mbp && global_mbp->write_halfword) {
		...
		//fprintf(stderr, "NumInstrs %I64u, mem_write_halfword addr = %x no bank\n",state->NumInstrs, addr);
		//chy 2003-09-03
		//SKYEYE_OUTREGS(stderr);
		//exit(-1);
	}
...
//      bank_ptr(addr)->write_word(state, addr, data);
	//mem_bank_t *mbp = bank_ptr(addr);
	if (skyeye_config.code_cov.prof_on) cov_prof(WRITE_FLAG, addr);
	...
	if (global_mbp && global_mbp->write_word) {
		...
		//fprintf(stderr, "NumInstrs %I64u, mem_write_word addr = %x no bank\n",state->NumInstrs, addr);
		//chy 2003-09-03
		//SKYEYE_OUTREGS(stderr);
		//exit(-1);
	}
...
void warn_write_byte(ARMul_State * state, ARMword addr, ARMword data) {
	fprintf(stderr, "WARNING: illegal write byte to 0x%x of 0x%x @ 0x%x\n", addr, data, state->pc);
}

void warn_write_halfword(ARMul_State * state, ARMword addr, ARMword data) {
	fprintf(stderr, "WARNING: illegal write halfword to 0x%x of 0x%x @ 0x%x\n", addr, data, state->pc);
}

void warn_write_word(ARMul_State * state, ARMword addr, ARMword data) {
	fprintf(stderr, "WARNING: illegal write word to 0x%x of 0x%x @ 0x%x\n", addr, data, state->pc);
}
...
ARMword real_read_byte(ARMul_State * state, ARMword addr) {
	...
	data = state->mem.rom[global_mbp - state->mem_bank->mem_banks][(addr - global_mbp->addr) >> 2];
...
void real_write_byte(ARMul_State * state, ARMword addr, ARMword data) {
	...
	temp = &state->mem.rom[global_mbp - state->mem_bank->mem_banks][(addr - global_mbp->addr) >> 2];
	offset = (((ARMword)state->bigendSig * 3) ^ (addr & 3)) << 3; /* bit offset into the word */
	//printf(stderr,"real_write_byte 1: temp %x,tempval %x,offset %x, addr %x, data %x\n",temp,*temp,offset,addr,data);

	*temp = (*temp & ~(0xffL << offset)) | ((data & 0xffL) << offset);
	//printf(stderr,"real_write_byte 2: temp %x,tempval %x,offset %x, addr %x, data %x\n",temp,*temp,offset,addr,data);
	...
	/*
	 ...
	 //fprintf(stderr, "lcd enabled  write byte lcd memory addr %x, data %x\n",addr,*temp);
...
void real_write_word(ARMul_State * state, ARMword addr, ARMword data) {
	...
	state->mem.rom[global_mbp - state->mem_bank->mem_banks][(addr - global_mbp->addr) >> 2] = data;
	//chy 2004-03-11: add lcd test
	//chy 2004-03-17 fix a bug: didn't test skyeye_config.no_lcd
	//chy 2004-09-29 disable blow lines
	/*
