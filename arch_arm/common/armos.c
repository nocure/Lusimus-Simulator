...
unsigned ARMul_OSInit(ARMul_State * state) {
	...
	for (i = FPESTART + fpesize;; i -= 4) { /* reverse the error strings */
		if ((j = ARMul_ReadWord(state, i)) == 0xffffffff) break;
		if (state->bigendSig && j < 0x80000000) { /* it's part of the string so swap it */
			j = ((j >> 0x18) & 0x000000ff) | ((j >> 0x08) & 0x0000ff00) | ((j << 0x08) & 0x00ff0000) | ((j << 0x18) & 0xff000000);
...
void ARMul_OSExit(ARMul_State * state) {
	if (state->OSptr) free((char *)state->OSptr);
}
...
static void SWIWrite0(ARMul_State * state, ARMword addr) {
	ARMword temp;
	struct OSblock *OSptr = (struct OSblock *)state->OSptr;
...
static void WriteCommandLineTo(ARMul_State * state, ARMword addr) {
	ARMword temp;
...
static void SWIopen(ARMul_State * state, ARMword name, ARMword SWIflags) {
	struct OSblock *OSptr = (struct OSblock *)state->OSptr;
	char dummy[2000];
	int flags;
	int i;
...
static void SWIread(ARMul_State * state, ARMword f, ARMword ptr, ARMword len) {
	struct OSblock *OSptr = (struct OSblock *)state->OSptr;
	int res;
	int i;
...
static void SWIwrite(ARMul_State * state, ARMword f, ARMword ptr, ARMword len) {
	struct OSblock *OSptr = (struct OSblock *)state->OSptr;
	int res;
	ARMword i;
	char *local = malloc(len);
...
static void SWIflen(ARMul_State * state, ARMword fh) {
	struct OSblock *OSptr = (struct OSblock *)state->OSptr;
	ARMword addr;
...
unsigned ARMul_OSHandleSWI(ARMul_State * state, ARMword number) {
	...
	case SWI_Clock:
		...
		state->Reg[0] =
				...
				(CLOCKS_PER_SEC >= 100) ? (ARMword)(clock() / (CLOCKS_PER_SEC / 100)) : (ARMword)((clock() * 100) / CLOCKS_PER_SEC);
#else
	...
	case SWI_Seek: {
		...
		state->Reg[0] = -1 >= lseek(state->Reg[0], state->Reg[1],
		SEEK_SET);
		OSptr->ErrorNo = errno;
		return TRUE;
	}
	...
	case AngelSWI_Thumb:
		...
		case AngelSWI_Reason_Clock:
			...
			state->Reg[0] =
#ifdef CLOCKS_PER_SEC
					(CLOCKS_PER_SEC >= 100) ? (ARMword)(clock() / (CLOCKS_PER_SEC / 100)) : (ARMword)((clock() * 100) /
					CLOCKS_PER_SEC);
#else
			/* presume unix... clock() returns microseconds */
			(ARMword) (clock () / 10000);
		...
		case AngelSWI_Reason_Seek:
			state->Reg[0] = -1 >= lseek(ARMul_ReadWord(state, addr), ARMul_ReadWord(state, addr + 4),
			SEEK_SET);
			OSptr->ErrorNo = errno;
			return (TRUE);
		...
		case AngelSWI_Reason_Open:
			SWIopen(state, ARMul_ReadWord(state, addr), ARMul_ReadWord(state, addr + 4));
			return TRUE;

		case AngelSWI_Reason_Read:
			SWIread(state, ARMul_ReadWord(state, addr), ARMul_ReadWord(state, addr + 4), ARMul_ReadWord(state, addr + 8));
			return TRUE;

		case AngelSWI_Reason_Write:
			SWIwrite(state, ARMul_ReadWord(state, addr), ARMul_ReadWord(state, addr + 4), ARMul_ReadWord(state, addr + 8));
			return TRUE;
		}
...
unsigned ARMul_OSException(ARMul_State * state ATTRIBUTE_UNUSED, ARMword vector ATTRIBUTE_UNUSED, ARMword pc ATTRIBUTE_UNUSED) { /* don't use this here */
	return (FALSE);
}
