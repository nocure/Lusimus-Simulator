...
ARMword ARMul_GetSPSR(ARMul_State * state, ARMword mode) {
	...
	if (!BANK_CAN_ACCESS_SPSR(bank)) return ARMul_GetCPSR(state);
...
void ARMul_SetSPSR(ARMul_State * state, ARMword mode, ARMword value) {
	...
	if (BANK_CAN_ACCESS_SPSR(bank)) state->Spsr[bank] = value;
}
...
void ARMul_R15Altered(ARMul_State * state) {
	...
	if (state->Mode > SVC26MODE) state->Emulate = CHANGEMODE;
...
ARMword ARMul_SwitchMode(ARMul_State * state, ARMword oldmode, ARMword newmode) {
	...
	if (oldbank != newbank) {
		...
		case ABORTBANK:
		case UNDEFBANK:
			if (newbank == FIQBANK) for (i = 8; i < 13; i++)
		...
		case ABORTBANK:
		case UNDEFBANK:
			if (oldbank == FIQBANK) for (i = 8; i < 13; i++)
...
static ARMword ModeToBank(ARMword mode) {
	...
	if (mode >= (sizeof(bankofmode) / sizeof(bankofmode[0]))) return DUMMYBANK;
...
void ARMul_AddCarry(ARMul_State * state, ARMword a, ARMword b, ARMword result) {
	ASSIGNC((NEG (a) && NEG (b)) || (NEG (a) && POS (result)) || (NEG (b) && POS (result)));
}
...
void ARMul_SubCarry(ARMul_State * state, ARMword a, ARMword b, ARMword result) {
	ASSIGNC((NEG (a) && POS (b)) || (NEG (a) && POS (result)) || (POS (b) && POS (result)));
}
...
void ARMul_LDC(ARMul_State * state, ARMword instr, ARMword address) {
	...
	UNDEF_LSCPCBaseWb;
	//printf("ARMul_LDC, CPnum is %x, instr %x, addr %x\n",CPNum, instr, address);
...
//  printf("ARMul_LDC, should update this function!!!!!\n");
	//exit(-1);
	if (!CP_ACCESS_ALLOWED(state, CPNum)) {
		/*
		 printf
		 ("ARMul_LDC,NOT ALLOW, underinstr, CPnum is %x, instr %x, addr %x\n",
		 CPNum, instr, address);
		 */
	...
	if (cpab == ARMul_CANT) {
		/*
		 printf
		 ("ARMul_LDC,NOT CAN, underinstr, CPnum is %x, instr %x, addr %x\n",
		 CPNum, instr, address);
		 */
		CPTAKEABORT;
		return;
	...
	//chy 2004-05-25
	if (state->abortSig || state->Aborted) goto L_ldc_takeabort;
...
void ARMul_STC(ARMul_State * state, ARMword instr, ARMword address) {
	...
	//printf("ARMul_STC, CPnum is %x, instr %x, addr %x\n",CPNum, instr, address);
	/*chy 2004-05-23 should update this function in the future,should concern dataabort */
//  skyeye_instr_debug=0;printf("debug end!!!!\n");
// chy 2004-05-25 , fix it now,so needn't printf
//  printf("ARMul_STC, should update this function!!!!!\n");
	//exit(-1);
	if (!CP_ACCESS_ALLOWED(state, CPNum)) {
		/*
		 printf
		 ("ARMul_STC,NOT ALLOW, undefinstr,  CPnum is %x, instr %x, addr %x\n",
		 CPNum, instr, address);
		 */
		ARMul_UndefInstr(state, instr);
		return;
	...
	if (cpab == ARMul_CANT) {
		/*
		 printf
		 ("ARMul_STC,CANT, undefinstr,  CPnum is %x, instr %x, addr %x\n",
		 CPNum, instr, address);
		 */
		CPTAKEABORT;
		return;
...
//chy 2004-05-25
	...
	//chy 2004-05-25
	if (state->abortSig || state->Aborted) goto L_stc_takeabort;
...
void ARMul_MCR(ARMul_State * state, ARMword instr, ARMword source) {
	...
	//printf("ARMul_MCR, CPnum is %x, source %x\n",CPNum, source);
	if (!CP_ACCESS_ALLOWED(state, CPNum)) {
		//chy 2004-07-19 should fix in the future ????!!!!
		//printf("ARMul_MCR, ACCESS_not ALLOWed, UndefinedInstr  CPnum is %x, source %x\n",CPNum, source);
		ARMul_UndefInstr(state, instr);
		return;
	}
	...
	if (cpab == ARMul_CANT) {
		printf("ARMul_MCR, CANT, UndefinedInstr  CPnum is %x, source %x\n", CPNum, source);
...
ARMword ARMul_MRC(ARMul_State * state, ARMword instr) {
	...
	//printf("ARMul_MRC, CPnum is %x, instr %x\n",CPNum, instr);
	if (!CP_ACCESS_ALLOWED(state, CPNum)) {
		//chy 2004-07-19 should fix in the future????!!!!
		//printf("ARMul_MRC,NOT ALLOWed UndefInstr  CPnum is %x, instr %x\n",CPNum, instr);
		ARMul_UndefInstr(state, instr);
		return 0;
	}
	...
	if (cpab == ARMul_CANT) {
		printf("ARMul_MRC,CANT UndefInstr  CPnum is %x, instr %x\n", CPNum, instr);
		ARMul_Abort(state, ARMul_UndefinedInstrV);
...
void ARMul_UndefInstr(ARMul_State * state, ARMword instr ATTRIBUTE_UNUSED) {
	ARMul_Abort(state, ARMul_UndefinedInstrV);
}
...
ARMword ARMul_Align(state, address, data)
	ARMul_State *state ATTRIBUTE_UNUSED;ARMword address;ARMword data; {
...
void ARMul_ScheduleEvent(ARMul_State * state, unsigned int delay, unsigned (*what)(ARMul_State *)) {
	unsigned int when;
	struct EventNode *event;

	if (state->EventSet++ == 0) state->Now = ARMul_Time(state);
	when = (state->Now + delay) % EVENTLISTSIZE;
	event = (struct EventNode *)malloc(sizeof(struct EventNode));
	if (!event) {
		printf("ARMul_ScheduleEvent: malloc event error\n");
		skyeye_exit(-1);
	}
	event->func = what;
...
static void EnvokeList(ARMul_State * state, unsigned int from, unsigned int to) {
	for (; from <= to; from++) {
...
unsigned int ARMul_Time(ARMul_State * state) {
	return (state->NumScycles + state->NumNcycles + state->NumIcycles + state->NumCcycles + state->NumFcycles);
}
