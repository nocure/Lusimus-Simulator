...
void ARMul_InitSymTable(char *filename) {
	...
	if (!filename) {
		printf("Can not get correct kernel filename! Maybe your .conf have something wrong!\n");
		skyeye_exit(-1);
	}
	/* <tktan> BUG200105221946 : get symbol from usrappl */
...
//	ARMword new_task_id;
	...
	if (to_pc == (tsp->func_stack[level].ret_addr + 4)) {
		if (state->energy.func_display & state->energy.func_disp_start) { /* <tktan> BUG200104191428 */
			//if(1){
			printf("[%x:%d:%x] Function return %s (%x)--> %s (%x)\n", tsp->task_id, level, fp_value, tsp->func_stack[level].func_symbol->name, cur_pc, tsp->func_stack[level
					- 1].func_symbol->name, to_pc);
		}
	...
	while ((level - bt >= 0) && (bt <= 3)) {
		if (to_pc == tsp->func_stack[level - bt].ret_addr) {
			if (state->energy.func_display & state->energy.func_disp_start) { /* <tktan> BUG200104191428 */
				printf("[%x:%d:%x] Trap Return -> %s\n", tsp->task_id, level, fp_value, tsp->func_stack[level - bt - 1].func_symbol->name);
			}
	...
	while ((level - bt >= 0) && (bt <= 2)) {
		if (to_pc == tsp->func_stack[level - bt].func_start_addr) {
			if (state->energy.func_display & state->energy.func_disp_start) { /* <tktan> BUG200104191428 */
				printf("[%x:%d:%x] Function %s ended\n", tsp->task_id, level, fp_value, tsp->func_stack[level - bt].func_symbol->name);
			}
	...
	if (!(to_pc & 0xffffffe0)) {	// check for pc from 0x0 - 0x1f
		...
		if (state->energy.func_display & state->energy.func_disp_start) { /* <tktan> BUG200104191428 */
			printf("[%x:%d:%x] Function %s(%x) --> Trap %x\n", tsp->task_id, level, fp_value, tsp->func_stack[level].func_symbol->name, cur_pc, to_pc);
		}
		return;
	}
...
void ARMul_ReportEnergy(ARMul_State * state, FILE * pf) {
	...
	do {
		...
		fprintf(pf, "Task[%x] %I64d %f\n", ctsp->task_id, ctsp->total_cycle, energy);
		ctsp = ctsp->next;
	} while (ctsp != oldtsp);
}
