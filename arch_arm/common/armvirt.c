...
*        Get a halfword into Virtual Memory, maybe allocating the page          *
 \***************************************************************************/
static fault_t GetHalfWord(ARMul_State * state, ARMword address, ARMword * data) {
	fault_t fault;

	fault = mmu_read_halfword(state, address, data);
	if (fault) {
//chy 2003-07-11: sometime has fault, but linux can continue running  !!!!????
//              printf("GetHalfWord fault %d \n", fault);
	}
	return fault;
}
...
static fault_t GetWord(ARMul_State * state, ARMword address, ARMword * data) {
	fault_t fault;
 ...
 * 			Load a Instrion Word into Virtual Memory						*
 \****************************************************************************/
static fault_t LoadInstr(ARMul_State * state, ARMword address, ARMword * instr) {
	fault_t fault;
...
*        Put a byte into Virtual Memory, maybe allocating the page          *
 \***************************************************************************/
static fault_t PutByte(ARMul_State * state, ARMword address, ARMword data) {
	fault_t fault;

	fault = mmu_write_byte(state, address, data);
	if (fault) {
//chy 2003-07-11: sometime has fault, but linux can continue running  !!!!????
//              printf("PutByte fault %d \n", fault);
	}
	return fault;
}
...
*        Put a halfword into Virtual Memory, maybe allocating the page          *
 \***************************************************************************/
static fault_t PutHalfWord(ARMul_State * state, ARMword address, ARMword data) {
	fault_t fault;

	fault = mmu_write_halfword(state, address, data);
	if (fault) {
//chy 2003-07-11: sometime has fault, but linux can continue running  !!!!????
//              printf("PutHalfWord fault %d \n", fault);
	}
	return fault;
}
...
static fault_t PutWord(ARMul_State * state, ARMword address, ARMword data) {
	fault_t fault;
...
unsigned ARMul_MemoryInit(ARMul_State * state, unsigned int initmemsize) {
	return TRUE;
}
...
int ARMul_ICE_ReadByte(ARMul_State * state, ARMword address, ARMword *presult) {
	...
	if (fault) {
		...
		*(char *)presult = (unsigned char)(data & 0xff);
		fault = 0;
		return fault;
	}
}
...
ARMword ARMul_LoadByte(ARMul_State * state, ARMword address) {
	state->NumNcycles++;
// ww 2009-016 fixed: added &0xff to make sure a byte
	return ARMul_ReadByte(state, address)/*&0xff*/;
}
...
void ARMul_Ccycles(ARMul_State * state, unsigned number, ARMword address ATTRIBUTE_UNUSED) {
