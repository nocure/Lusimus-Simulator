...
static fault_t a71_mmu_read(ARMul_State * state, ARMword virt_addr, ARMword * data, ARMword datatype);
static fault_t a71_mmu_write(ARMul_State * state, ARMword virt_addr, ARMword data, ARMword datatype);
static int a71_mmu_init(ARMul_State * state) {
...
static void a71_mmu_exit(ARMul_State * state) {
...
static fault_t a71_mmu_read_halfword(ARMul_State * state, ARMword virt_addr, ARMword * data) {
...
static fault_t a71_mmu_read_word(ARMul_State * state, ARMword virt_addr, ARMword * data) {
...
//#if 0
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("1 a71_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
...
//#endif
...
#endif /*  */
	if (((virt_addr & 3) && (datatype == ARM_WORD_TYPE) && (state->mmu.control & CONTROL_ALIGN_FAULT))
			|| ((virt_addr & 1) && (datatype == ARM_HALFWORD_TYPE) && (state->mmu.control & CONTROL_ALIGN_FAULT))) {
		printf("a71_mmu_read ALIGNMENT_FAULT\n");
	...
	if ((tlb->perms & 0x08) && (state->mmu.control & CONTROL_CACHE)) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("2 a71_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
		return NO_FAULT;
	}
}
static fault_t a71_mmu_write_byte(ARMul_State * state, ARMword virt_addr, ARMword data) {
	return a71_mmu_write(state, virt_addr, data, ARM_BYTE_TYPE);
}

static fault_t a71_mmu_write_halfword(ARMul_State * state, ARMword virt_addr, ARMword data) {
	return a71_mmu_write(state, virt_addr, data, ARM_HALFWORD_TYPE);
}

static fault_t a71_mmu_write_word(ARMul_State * state, ARMword virt_addr, ARMword data) {
...
static fault_t a71_mmu_write(ARMul_State * state, ARMword virt_addr, ARMword data, ARMword datatype) {
	...
	if (!(state->mmu.control & CONTROL_MMU)) {
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("1 a71_mmu_write error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
		return NO_FAULT;
	}
...
//      if ((virt_addr & 3) && (state->mmu.control & CONTROL_ALIGN_FAULT)) {
	if (((virt_addr & 3) && (datatype == ARM_WORD_TYPE) && (state->mmu.control & CONTROL_ALIGN_FAULT))
			|| ((virt_addr & 1) && (datatype == ARM_HALFWORD_TYPE) && (state->mmu.control & CONTROL_ALIGN_FAULT))) {
		printf("a71_mmu_write ALIGNMENT_FAULT\n");
		return ALIGNMENT_FAULT;
	}
	if (state->mmu.control & CONTROL_CACHE) {
		cache_line_t *cache;
		...
		if (cache) {
			if (datatype == ARM_WORD_TYPE)
				...
				temp = cache->data[(virt_addr >> 2) & 3];
				offset = (((ARMword)state->bigendSig * 2) ^ (virt_addr & 2)) << 3; /* bit offset into the word */
				cache->data[(virt_addr >> 2) & 3] = (temp & ~(0xffffL << offset)) | ((data & 0xffffL) << offset);
			}
			else if (datatype == ARM_BYTE_TYPE) {
				temp = cache->data[(virt_addr >> 2) & 3];
				offset = (((ARMword)state->bigendSig * 3) ^ (virt_addr & 3)) << 3; /* bit offset into the word */
				cache->data[(virt_addr >> 2) & 3] = (temp & ~(0xffL << offset)) | ((data & 0xffL) << offset);
			}
		}
	}
	fault = translate(state, virt_addr, TLB (), &tlb);
	...
	if (datatype == ARM_BYTE_TYPE)
		...
		printf("2  a71_mmu_write error: unknown data type %d \n", datatype);
		skyeye_exit(-1);
	}
	return NO_FAULT;
}

static ARMword a71_mmu_mrc(ARMul_State * state, ARMword instr, ARMword * value) {
...
static ARMword a71_mmu_mcr(ARMul_State * state, ARMword instr, ARMword value) {
	mmu_regnum_t creg = BITS (16, 19) & 15;
...
static int a71_mmu_v2p_dbct(ARMul_State * state, ARMword virt_addr, ARMword * phys_addr) {
	tlb_entry_t *tlb;
//	ARMword temp, offset;
	fault_t fault;
...
#undef CACHE
#undef TLB
mmu_ops_t arm7100_mmu_ops = { a71_mmu_init, a71_mmu_exit, a71_mmu_read_byte, a71_mmu_write_byte, a71_mmu_read_halfword, a71_mmu_write_halfword,
		a71_mmu_read_word, a71_mmu_write_word, a71_mmu_read_word,	// load instr
