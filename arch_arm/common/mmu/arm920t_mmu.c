...
static arm920t_mmu_desc_t arm920t_mmu_desc = { 64,
/* 32 bytes per line
 * 64 way
 * 8 set
 * */
...
64, { 32, 64, 8, CACHE_WRITE_BACK }, { 8, 8 }			//for byte size
};

static fault_t arm920t_mmu_write(ARMul_State * state, ARMword va, ARMword data, ARMword datatype);
static fault_t arm920t_mmu_read(ARMul_State * state, ARMword va, ARMword * data, ARMword datatype);
...
int arm920t_mmu_init(ARMul_State * state) {
	...
	c_desc = &desc->i_cache;
	if (mmu_cache_init(ARM920T_I_CACHE (), c_desc->width, c_desc->way, c_desc->set, c_desc->w_mode)) {
	...
	c_desc = &desc->d_cache;
	if (mmu_cache_init(ARM920T_D_CACHE (), c_desc->width, c_desc->way, c_desc->set, c_desc->w_mode)) {
	...
	wb_init_error: mmu_cache_exit(ARM920T_D_CACHE ());
	d_cache_init_error: mmu_tlb_exit(ARM920T_D_TLB ());
	d_tlb_init_error: mmu_cache_exit(ARM920T_I_CACHE ());
	i_cache_init_error: mmu_tlb_exit(ARM920T_I_TLB ());
	i_tlb_init_error: return -1;
}

void arm920t_mmu_exit(ARMul_State * state) {
...
static fault_t arm920t_mmu_load_instr(ARMul_State * state, ARMword va, ARMword * instr) {
	fault_t fault;
	tlb_entry_t *tlb;
	cache_line_t *cache;
	int c;			//cache bit
...
static fault_t arm920t_mmu_read_byte(ARMul_State * state, ARMword virt_addr, ARMword * data) {
	//ARMword temp,offset;
	fault_t fault;
...
static fault_t arm920t_mmu_read_halfword(ARMul_State * state, ARMword virt_addr, ARMword * data) {
	//ARMword temp,offset;
	fault_t fault;
...
static fault_t arm920t_mmu_read_word(ARMul_State * state, ARMword virt_addr, ARMword * data) {
...
static fault_t arm920t_mmu_read(ARMul_State * state, ARMword va, ARMword * data, ARMword datatype) {
	...
	if (MMU_Disabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("1 arm920t_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
	...
	if (tlb_c_flag (tlb) && MMU_DCacheEnabled) {
		...
		*data = cache->data[va_cache_index(va, cache_t)];
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("2 arm920t_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
		return 0;
	}
...
static fault_t arm920t_mmu_write_byte(ARMul_State * state, ARMword virt_addr, ARMword data) {
	return arm920t_mmu_write(state, virt_addr, data, ARM_BYTE_TYPE);
}

static fault_t arm920t_mmu_write_halfword(ARMul_State * state, ARMword virt_addr, ARMword data) {
	return arm920t_mmu_write(state, virt_addr, data, ARM_HALFWORD_TYPE);
}

static fault_t arm920t_mmu_write_word(ARMul_State * state, ARMword virt_addr, ARMword data) {
...
static fault_t arm920t_mmu_write(ARMul_State * state, ARMword va, ARMword data, ARMword datatype) {
	...
	if (MMU_Disabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("1 arm920t_mmu_write error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
		...
		if (b) {
			if (MMU_WBEnabled) {
				if (datatype == ARM_WORD_TYPE)
					...
					ARM920T_WB (), pa, (ARMbyte*)&data, 4);
				else if (datatype == ARM_HALFWORD_TYPE)
					mmu_wb_write_bytes(state,
					ARM920T_WB (), (pa | (real_va & 2)), (ARMbyte*)&data, 2);
				else if (datatype == ARM_BYTE_TYPE) mmu_wb_write_bytes(state,
				ARM920T_WB (), (pa | (real_va & 3)), (ARMbyte*)&data, 1);
...
ARMword arm920t_mmu_mrc(ARMul_State * state, ARMword instr, ARMword * value) {
	...
	case MMU_FAULT_STATUS:
		 ...
		 * */
		if (OPC_2 == 0) data = state->mmu.fault_status;
		break;
 ...
 * */
void arm920t_mmu_cache_ops(ARMul_State * state, ARMword instr, ARMword value) {
	int CRm, OPC_2;
...
static void arm920t_mmu_tlb_ops(ARMul_State * state, ARMword instr, ARMword value) {
	int CRm, OPC_2;
...
static ARMword arm920t_mmu_mcr(ARMul_State * state, ARMword instr, ARMword value) {
...
static int arm920t_mmu_v2p_dbct(ARMul_State * state, ARMword virt_addr, ARMword * phys_addr) {
	fault_t fault;
	tlb_entry_t *tlb;
...
/*arm920t mmu_ops_t*/
mmu_ops_t arm920t_mmu_ops = { arm920t_mmu_init, arm920t_mmu_exit, arm920t_mmu_read_byte, arm920t_mmu_write_byte, arm920t_mmu_read_halfword,
		arm920t_mmu_write_halfword, arm920t_mmu_read_word, arm920t_mmu_write_word, arm920t_mmu_load_instr, arm920t_mmu_mcr, arm920t_mmu_mrc,
//teawater add for arm2x86 2005.06.19-------------------------------------------
