...
static fault_t arm926ejs_mmu_write(ARMul_State * state, ARMword va, ARMword data, ARMword datatype);
static fault_t arm926ejs_mmu_read(ARMul_State * state, ARMword va, ARMword * data, ARMword datatype);
static fault_t update_cache(ARMul_State * state, ARMword va, ARMword data, ARMword datatype, cache_line_t * cache, cache_t * cache_t, ARMword real_va);

int arm926ejs_mmu_init(ARMul_State * state) {
	...
	c_desc = &desc->i_cache;
	if (mmu_cache_init(ARM926EJS_I_CACHE (), c_desc->width, c_desc->way, c_desc->set, c_desc->w_mode)) {
	...
	c_desc = &desc->d_cache;
	if (mmu_cache_init(ARM926EJS_D_CACHE (), c_desc->width, c_desc->way, c_desc->set, c_desc->w_mode)) {
	...
	lockdown_tlb_init_error: mmu_cache_exit(ARM926EJS_I_CACHE ());
	i_cache_init_error: mmu_tlb_exit(ARM926EJS_MAIN_TLB ());
	main_tlb_init_error: return -1;
}

void arm926ejs_mmu_exit(ARMul_State * state) {
...
static fault_t arm926ejs_mmu_load_instr(ARMul_State * state, ARMword va, ARMword * instr) {
	fault_t fault;
	tlb_entry_t *tlb;
	cache_line_t *cache;
	int c;			//cache bit
	...
	if (cache) {
		*instr = cache->data[va_cache_index(va, ARM926EJS_I_CACHE ())];
		return 0;
	}
...
static fault_t arm926ejs_mmu_read_byte(ARMul_State * state, ARMword virt_addr, ARMword * data) {
	//ARMword temp,offset;
	fault_t fault;
...
static fault_t arm926ejs_mmu_read_halfword(ARMul_State * state, ARMword virt_addr, ARMword * data) {
	//ARMword temp,offset;
	fault_t fault;
...
static fault_t arm926ejs_mmu_read_word(ARMul_State * state, ARMword virt_addr, ARMword * data) {
...
static fault_t arm926ejs_mmu_read(ARMul_State * state, ARMword va, ARMword * data, ARMword datatype) {
	...
	if (MMU_Disabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("1 arm926ejs_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
	...
	if (cache) {
		*data = cache->data[va_cache_index(va, ARM926EJS_D_CACHE ())];
		goto datatrans;
		//return 0;
	}
	...
	if (tlb_b_flag(tlb)) mmu_wb_drain_all(state, ARM926EJS_WB ());
	...
	if (tlb_c_flag (tlb) && MMU_DCacheEnabled) {
		...
		*data = cache->data[va_cache_index(va, cache_t)];
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("2 arm926ejs_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
		return 0;
	}
...
static fault_t arm926ejs_mmu_write_byte(ARMul_State * state, ARMword virt_addr, ARMword data) {
	return arm926ejs_mmu_write(state, virt_addr, data, ARM_BYTE_TYPE);
}

static fault_t arm926ejs_mmu_write_halfword(ARMul_State * state, ARMword virt_addr, ARMword data) {
	return arm926ejs_mmu_write(state, virt_addr, data,
	ARM_HALFWORD_TYPE);
}

static fault_t arm926ejs_mmu_write_word(ARMul_State * state, ARMword virt_addr, ARMword data) {
...
static fault_t arm926ejs_mmu_write(ARMul_State * state, ARMword va, ARMword data, ARMword datatype) {
	...
	if (MMU_Disabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("1 arm926ejs_mmu_write error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
		...
		if (b) {
			if (MMU_WBEnabled) {
				if (datatype == ARM_WORD_TYPE)
					...
					ARM926EJS_WB (), pa, (ARMbyte*)&data, 4);
				else if (datatype == ARM_HALFWORD_TYPE)
					mmu_wb_write_bytes(state,
					ARM926EJS_WB (), (pa | (real_va & 2)), (ARMbyte*)&data, 2);
				else if (datatype == ARM_BYTE_TYPE) mmu_wb_write_bytes(state,
				ARM926EJS_WB (), (pa | (real_va & 3)), (ARMbyte*)&data, 1);
...
ARMword arm926ejs_mmu_mrc(ARMul_State * state, ARMword instr, ARMword * value) {
	...
	case MMU_FAULT_STATUS:
		 ...
		 * */
		if (OPC_2 == 0) data = state->mmu.fault_status & 0x0FF;
		break;
 ...
 * */
void arm926ejs_mmu_cache_ops(ARMul_State * state, ARMword instr, ARMword value) {
	int CRm, OPC_2;
	...
	if (OPC_2 == 2 && CRm == 5) {
		mmu_cache_invalidate_by_index(state, ARM926EJS_I_CACHE (), value);
		return;
	}
	/* TODO:
...
static void arm926ejs_mmu_tlb_ops(ARMul_State * state, ARMword instr, ARMword value) {
	int CRm, OPC_2;
	...
	if (OPC_2 == 1 && ((CRm == 0x5) || (CRm == 0x7) || (CRm == 0x6))) {
		mmu_tlb_invalidate_entry(state, ARM926EJS_MAIN_TLB (), value);
		return;
	}
...
static ARMword arm926ejs_mmu_mcr(ARMul_State * state, ARMword instr, ARMword value) {
...
//              printf("mmu_mcr wrote DACR         ");
	...
	case MMU_FAULT_STATUS:
		if (OPC_2 == 0) state->mmu.fault_status = value & 0xFF;
		break;
	case MMU_FAULT_ADDRESS:
...
static int arm926ejs_mmu_v2p_dbct(ARMul_State * state, ARMword virt_addr, ARMword * phys_addr) {
	fault_t fault;
	tlb_entry_t *tlb;
	...
	if (MMU_Enabled) {
		...
		/*translate tlb */
		fault = translate(state, virt_addr, ARM926EJS_MAIN_TLB (), &tlb);
		if (fault) {
			d_msg ("translate\n");
			return fault;
		}
...
/*arm926ejs mmu_ops_t*/
mmu_ops_t arm926ejs_mmu_ops = { arm926ejs_mmu_init, arm926ejs_mmu_exit, arm926ejs_mmu_read_byte, arm926ejs_mmu_write_byte, arm926ejs_mmu_read_halfword,
		arm926ejs_mmu_write_halfword, arm926ejs_mmu_read_word, arm926ejs_mmu_write_word, arm926ejs_mmu_load_instr, arm926ejs_mmu_mcr, arm926ejs_mmu_mrc,
//teawater add for arm2x86 2005.06.19-------------------------------------------
