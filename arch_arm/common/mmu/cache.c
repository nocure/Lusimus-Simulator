...
cache_line_t *mmu_cache_search(ARMul_State * state, cache_t * cache_t, ARMword va) {
	...
	for (i = 0, cache = cache_set->lines; i < cache_t->way; i++, cache++) {
		if ((cache->tag & TAG_VALID_FLAG) && (tag == va_cache_align(cache->tag, cache_t))) return cache;
	}
	return NULL;
}
 ...
 * 		cache	:cache matched
 */
cache_line_t *mmu_cache_search_by_index(ARMul_State * state, cache_t * cache_t, ARMword index) {
	int way = cache_t->way;
...
cache_line_t *mmu_cache_alloc(ARMul_State * state, cache_t * cache_t, ARMword va, ARMword pa) {
	...
	if (set->cycle == cache_t->way) set->cycle = 0;
...
void mmu_cache_clean(ARMul_State * state, cache_t * cache_t, ARMword va) {
	...
	if (cache) mmu_cache_write_back(state, cache_t, cache);
}
 ...
 */
void mmu_cache_clean_by_index(ARMul_State * state, cache_t * cache_t, ARMword index) {
	cache_line_t *cache;
 ...
 */
void mmu_cache_invalidate_by_index(ARMul_State * state, cache_t * cache_t, ARMword index) {
	cache_line_t *cache;
