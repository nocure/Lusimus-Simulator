...
static void printf_nothing(void *foo, ...) {
}

static void cirrus_not_implemented(char *insn) {
...
unsigned DSPMRC4(ARMul_State * state ATTRIBUTE_UNUSED, unsigned type ATTRIBUTE_UNUSED, ARMword instr, ARMword * value) {
	switch (BITS(5, 7)) {
	case 0: /* cfmvrdl */
...
unsigned DSPMRC5(ARMul_State * state ATTRIBUTE_UNUSED, unsigned type ATTRIBUTE_UNUSED, ARMword instr, ARMword * value) {
	switch (BITS(5, 7)) {
	case 0: /* cfmvr64l */
	...
	case 4: /* cfcmp32 */
	{
		...
		n = res < 0;
		/* overflow */
		v = SubOverflow(DSPregs[SRC1_REG].lower.i, DSPregs[SRC2_REG].lower.i, res);
		/* carry */
		c = ((NEG (a) && POS(b)) || (NEG (a) && POS(res)) || (POS (b) && POS(res)));
	...
	case 5: /* cfcmp64 */
	{
		...
		n = res < 0;
		/* overflow */
		v = ((NEG64 (a) && POS64(b) && POS64(res)) || (POS64 (a) && NEG64(b) && NEG64(res)));
		/* carry */
		c = ((NEG64 (a) && POS64(b)) || (NEG64 (a) && POS64(res)) || (POS64 (b) && POS64(res)));
...
unsigned DSPMRC6(ARMul_State * state ATTRIBUTE_UNUSED, unsigned type ATTRIBUTE_UNUSED, ARMword instr, ARMword * value) {
	switch (BITS(5, 7)) {
	case 0: /* cfmval32 */
...
unsigned DSPMCR4(ARMul_State * state, unsigned type ATTRIBUTE_UNUSED, ARMword instr, ARMword value) {
	switch (BITS(5, 7)) {
	case 0: /* cfmvdlr */
...
unsigned DSPMCR5(ARMul_State * state, unsigned type ATTRIBUTE_UNUSED, ARMword instr, ARMword value) {
	union {
		int s;
		unsigned int us;
	} val;
...
unsigned DSPMCR6(ARMul_State * state, unsigned type ATTRIBUTE_UNUSED, ARMword instr, ARMword value) {
	switch (BITS(5, 7)) {
	case 0: /* cfmv32al */
...
unsigned DSPLDC4(ARMul_State * state ATTRIBUTE_UNUSED, unsigned type, ARMword instr, ARMword data) {
	...
	if (BIT(22)) { /* it's a long access, get two words */
		...
		printfdbg("\tmvf%d <-- mem = %f\n", DEST_REG, DSPregs[DEST_REG].upper.f);

		return ARMul_DONE;
	}
}

unsigned DSPLDC5(ARMul_State * state ATTRIBUTE_UNUSED, unsigned type, ARMword instr, ARMword data) {
	static unsigned words;
	...
	if (BIT(22)) {
		...
		if (words == 2) {
			printfdbg("\tmvdx%d <-- mem = %I64d\n", DEST_REG, mv_getReg64int(DEST_REG));
...
unsigned DSPSTC4(ARMul_State * state ATTRIBUTE_UNUSED, unsigned type, ARMword instr, ARMword * data) {
	static unsigned words;
...
unsigned DSPSTC5(ARMul_State * state ATTRIBUTE_UNUSED, unsigned type, ARMword instr, ARMword * data) {
	static unsigned words;
...
unsigned DSPCDP4(ARMul_State * state, unsigned type, ARMword instr) {
	int opcode2;
	...
	case 1:
		...
		case 0: /* cfmuls */
			...
			DEST_REG,
			SRC1_REG, DSPregs[SRC1_REG].upper.f * DSPregs[SRC2_REG].upper.f);

			DSPregs[DEST_REG].upper.f = DSPregs[SRC1_REG].upper.f * DSPregs[SRC2_REG].upper.f;
			break;

		case 1: /* cfmuld */
			printfdbg("cfmuld mvd%d = mvd%d = %g\n",
			DEST_REG,
			SRC1_REG, mv_getRegDouble(SRC1_REG) * mv_getRegDouble(SRC2_REG));

			mv_setRegDouble(DEST_REG, mv_getRegDouble(SRC1_REG) * mv_getRegDouble(SRC2_REG));
			break;
	...
	case 3:
		switch (opcode2) {
		case 0: /* cfabss */
			DSPregs[DEST_REG].upper.f = (DSPregs[SRC1_REG].upper.f < 0.0F ? -DSPregs[SRC1_REG].upper.f : DSPregs[SRC1_REG].upper.f);
			printfdbg("cfabss mvf%d = |mvf%d| = %f\n", DEST_REG,
			SRC1_REG, DSPregs[DEST_REG].upper.f);
			break;

		case 1: /* cfabsd */
			mv_setRegDouble(DEST_REG, (mv_getRegDouble(SRC1_REG) < 0.0 ? -mv_getRegDouble(SRC1_REG) : mv_getRegDouble(SRC1_REG)));
...
unsigned DSPCDP5(ARMul_State * state, unsigned type, ARMword instr) {
	int opcode2;
	char shift;
	...
	case 1:
		switch (opcode2) {
		case 0: /* cfmul32 */
			DSPregs[DEST_REG].lower.i = DSPregs[SRC1_REG].lower.i * DSPregs[SRC2_REG].lower.i;
			printfdbg("cfmul32 mvfx%d = mvfx%d * mvfx%d = %d\n",
			DEST_REG, SRC1_REG, SRC2_REG, DSPregs[DEST_REG].lower.i);
			break;

		case 1: /* cfmul64 */
			mv_setReg64int(DEST_REG, mv_getReg64int(SRC1_REG) * mv_getReg64int(SRC2_REG));
			printfdbg("cfmul64 mvdx%d = mvdx%d * mvdx%d = %I64d\n",
	...
	case 3:
		switch (opcode2) {
		case 0: /* cfabs32 */
			DSPregs[DEST_REG].lower.i = (DSPregs[SRC1_REG].lower.i < 0 ? -DSPregs[SRC1_REG].lower.i : DSPregs[SRC1_REG].lower.i);
			printfdbg("cfabs32 mvfx%d = |mvfx%d| = %d\n",
			DEST_REG, SRC1_REG, SRC2_REG, DSPregs[DEST_REG].lower.i);
			break;

		case 1: /* cfabs64 */
			mv_setReg64int(DEST_REG, (mv_getReg64int(SRC1_REG) < 0 ? -mv_getReg64int(SRC1_REG) : mv_getReg64int(SRC1_REG)));
			printfdbg("cfabs64 mvdx%d = |mvdx%d| = %I64d\n",
			DEST_REG, SRC1_REG, SRC2_REG, mv_getReg64int(DEST_REG));
			break;
		...
		case 3: /* cfneg64 */
			...
			printfdbg("cfneg64 mvdx%d = -mvdx%d = %I64d\n",
			DEST_REG, SRC1_REG, SRC2_REG, mv_getReg64int(DEST_REG));
			break;
		...
		case 5: /* cfadd64 */
			mv_setReg64int(DEST_REG, mv_getReg64int(SRC1_REG) + mv_getReg64int(SRC2_REG));
			printfdbg("cfadd64 mvdx%d = mvdx%d + mvdx%d = %I64d\n",
			DEST_REG, SRC1_REG, SRC2_REG, mv_getReg64int(DEST_REG));
			break;
...
unsigned DSPCDP6(ARMul_State * state, unsigned type, ARMword instr) {
//	int opcode2;
...
void mv_compute_host_endianness(ARMul_State * state) {
	static union {
		long long ll;
		int ints[2];
		int i;
