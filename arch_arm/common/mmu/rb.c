...
ARMword rb_masks[] = { 0x0,			//RB_INVALID
		4,			//RB_1
		16,			//RB_4
		32,			//RB_8
		};
...
int mmu_rb_init(rb_t * rb_t, int num) {
	...
	if (entrys == NULL) {
		printf("mmu_rb_init malloc error\n");
		return -1;
	}
	for (i = 0; i < num; i++) {
...
mmu_rb_search(rb_t * rb_t, ARMword va) {
	...
	for (i = 0; i < rb_t->num; i++, rb++) {
		...
		if (rb->type) {
			if ((va >= rb->va) && (va < (rb->va + rb_masks[rb->type]))) return rb;
		}
	}
	return NULL;
}
...
void mmu_rb_load(ARMul_State * state, rb_t * rb_t, int i_rb, int type, ARMword va) {
	rb_entry_t *rb;
	int i;
	...
	for (rb = rb_t->entrys, i = 0; i < rb_t->num; i++, rb++) {
		if (rb->type) {
			...
			min_end = min(va + rb_masks[type], rb->va + rb_masks[rb->type]);
			if (max_start < min_end) rb->type = RB_INVALID;
		}
	}
	/*load word */
	rb = &rb_t->entrys[i_rb];
