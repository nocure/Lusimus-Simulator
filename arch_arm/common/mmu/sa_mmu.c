...
void sa_mmu_exit(ARMul_State * state) {
	mmu_rb_exit(RB ());
	mmu_wb_exit(WB ());
...
static fault_t sa_mmu_load_instr(ARMul_State * state, ARMword va, ARMword * instr) {
	fault_t fault;
	tlb_entry_t *tlb;
	cache_line_t *cache;
	int c;			//cache bit
...
static fault_t sa_mmu_read_byte(ARMul_State * state, ARMword virt_addr, ARMword * data) {
	//ARMword temp,offset;
	fault_t fault;
...
static fault_t sa_mmu_read_halfword(ARMul_State * state, ARMword virt_addr, ARMword * data) {
	//ARMword temp,offset;
	fault_t fault;
...
static fault_t sa_mmu_read_word(ARMul_State * state, ARMword virt_addr, ARMword * data) {
	return sa_mmu_read(state, virt_addr, data, ARM_WORD_TYPE);
}

static fault_t sa_mmu_read(ARMul_State * state, ARMword va, ARMword * data, ARMword datatype) {
	fault_t fault;
	rb_entry_t *rb;
	tlb_entry_t *tlb;
	cache_line_t *cache;
	...
	if (MMU_Disabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("1 sa_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
	...
	if (tlb_b_flag(tlb)) mmu_wb_drain_all(state, WB ());
	...
	if (tlb_c_flag (tlb) && MMU_DCacheEnabled) {
		...
		*data = cache->data[va_cache_index(va, cache_t)];
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("2 sa_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
		return 0;
	}
...
static fault_t sa_mmu_write_byte(ARMul_State * state, ARMword virt_addr, ARMword data) {
	return sa_mmu_write(state, virt_addr, data, ARM_BYTE_TYPE);
}

static fault_t sa_mmu_write_halfword(ARMul_State * state, ARMword virt_addr, ARMword data) {
	return sa_mmu_write(state, virt_addr, data, ARM_HALFWORD_TYPE);
}

static fault_t sa_mmu_write_word(ARMul_State * state, ARMword virt_addr, ARMword data) {
	return sa_mmu_write(state, virt_addr, data, ARM_WORD_TYPE);
}

static fault_t sa_mmu_write(ARMul_State * state, ARMword va, ARMword data, ARMword datatype) {
	tlb_entry_t *tlb;
	cache_line_t *cache;
	int b;
	ARMword pa, real_va;
	...
	if (MMU_Disabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("1 sa_mmu_write error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
		...
		if (b) {
			if (MMU_WBEnabled) {
				if (datatype == ARM_WORD_TYPE)
					mmu_wb_write_bytes(state, WB (), pa, (ARMbyte*)&data, 4);
				else if (datatype == ARM_HALFWORD_TYPE)
					mmu_wb_write_bytes(state, WB (), (pa | (real_va & 2)), (ARMbyte*)&data, 2);
				else if (datatype == ARM_BYTE_TYPE) mmu_wb_write_bytes(state, WB (), (pa | (real_va & 3)), (ARMbyte*)&data, 1);
...
ARMword sa_mmu_mrc(ARMul_State * state, ARMword instr, ARMword * value) {
...
void sa_mmu_cache_ops(ARMul_State * state, ARMword instr, ARMword value) {
	int CRm, OPC_2;
...
static void sa_mmu_tlb_ops(ARMul_State * state, ARMword instr, ARMword value) {
	int CRm, OPC_2;
...
static void sa_mmu_rb_ops(ARMul_State * state, ARMword instr, ARMword value) {
	int CRm, OPC_2;
	...
	if (OPC_2 == 0x2) {
		...
		if ((idx < 4) && (type < 4)) mmu_rb_load(state, RB (), idx, type, value);
		return;
	}
...
static ARMword sa_mmu_mcr(ARMul_State * state, ARMword instr, ARMword value) {
	mmu_regnum_t creg = BITS (16, 19) & 15;
...
static int sa_mmu_v2p_dbct(ARMul_State * state, ARMword virt_addr, ARMword * phys_addr) {
	fault_t fault;
	tlb_entry_t *tlb;
