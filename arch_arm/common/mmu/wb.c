...
int mmu_wb_init(wb_t * wb_t, int num, int nb) {
	...
	data_malloc_error: while (--i >= 0)
		free(entrys[i].data);
	free(entrys);
	entrys_malloc_error: return -1;
}
;
 ...
 * */
void mmu_wb_write_bytes(ARMul_State * state, wb_t * wb_t, ARMword pa, ARMbyte * data, int n) {
	int i;
	wb_entry_t *wb;
	...
	while (n) {
		if (wb_t->num == wb_t->used) {
			...
			wb_t->last++;
			if (wb_t->last == wb_t->num) wb_t->last = 0;
			wb_t->used--;
		}
		...
		n -= i;
		wb_t->first++;
		if (wb_t->first == wb_t->num) wb_t->first = 0;
		wb_t->used++;
	};
