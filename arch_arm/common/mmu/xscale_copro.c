...
static fault_t xscale_mmu_write(ARMul_State * state, ARMword va, ARMword data, ARMword datatype);
static fault_t xscale_mmu_read(ARMul_State * state, ARMword va, ARMword * data, ARMword datatype);
...
unsigned xscale_cp6_mrc(ARMul_State * state, unsigned type, ARMword instr, ARMword * data) {
	unsigned opcode_2 = BITS(5, 7);
	unsigned CRm = BITS(0, 3);
	unsigned reg = BITS(16, 19);
//	unsigned result;

	//printf("xscale_cp6_mrc:opcode_2 0x%x, CRm 0x%x, reg 0x%x,reg[15] 0x%x, instr %x\n",opcode_2,CRm,reg,state->Reg[15], instr);
	...
	default:
		*data = 0;
		printf("cp6_mrc unknown cp6 regs!!!!!!\n");
		printf("xscale_cp6_mrc:opcode_2 0x%x, CRm 0x%x, reg 0x%x,reg[15] 0x%x, instr %x\n", opcode_2, CRm, reg, state->Reg[15], instr);
		break;
	}
	return 0;
...
//chy 2005-09-19 end
//xscale cp13 ----------------------------------------------------
unsigned xscale_cp13_init(ARMul_State * state) {
	//printf("xscale_cp13_init: begin\n");
	return 0;
}

unsigned xscale_cp13_exit(ARMul_State * state) {
	//printf("xscale_cp13_exit: begin\n");
	return 0;
}

__attribute__((noreturn)) unsigned xscale_cp13_ldc(ARMul_State * state, unsigned type, ARMword instr, ARMword data) {
	printf("xscale_cp13_ldc: ERROR isn't existed,");
	SKYEYE_OUTREGS(stderr);
	fprintf(stderr, "\n");
	skyeye_exit(-1);
}

unsigned xscale_cp13_stc(ARMul_State * state, unsigned type, ARMword instr, ARMword * data) {
	printf("xscale_cp13_stc: ERROR isn't existed,");
	SKYEYE_OUTREGS(stderr);
	fprintf(stderr, "\n");
	skyeye_exit(-1);
}

unsigned xscale_cp13_mrc(ARMul_State * state, unsigned type, ARMword instr, ARMword * data) {
	printf("xscale_cp13_mrc: ERROR isn't existed,");
	SKYEYE_OUTREGS(stderr);
	fprintf(stderr, "\n");
	skyeye_exit(-1);
}

unsigned xscale_cp13_mcr(ARMul_State * state, unsigned type, ARMword instr, ARMword data) {
	printf("xscale_cp13_mcr: ERROR isn't existed,");
	SKYEYE_OUTREGS(stderr);
	fprintf(stderr, "\n");
	skyeye_exit(-1);
}

unsigned xscale_cp13_cdp(ARMul_State * state, unsigned type, ARMword instr) {
	printf("xscale_cp13_cdp: ERROR isn't existed,");
	SKYEYE_OUTREGS(stderr);
	fprintf(stderr, "\n");
	skyeye_exit(-1);
}

unsigned xscale_cp13_read_reg(ARMul_State * state, unsigned reg, ARMword * data) {
	printf("xscale_cp13_read_reg: ERROR isn't existed,");
	SKYEYE_OUTREGS(stderr);
	fprintf(stderr, "\n");
	return 0;
	//exit(-1);
...
unsigned xscale_cp13_write_reg(ARMul_State * state, unsigned reg, ARMword data) {
	printf("xscale_cp13_write_reg: ERROR isn't existed,");
	SKYEYE_OUTREGS(stderr);
	fprintf(stderr, "\n");
	skyeye_exit(-1);
}
...
//------------------------------------------------------------------
//xscale cp14 ----------------------------------------------------
unsigned xscale_cp14_init(ARMul_State * state) {
	//printf("xscale_cp14_init: begin\n");
	pxa_cp14_regs.cclkcfg = 0;
	pxa_cp14_regs.pwrmode = 0;
	return 0;
}

unsigned xscale_cp14_exit(ARMul_State * state) {
	//printf("xscale_cp14_exit: begin\n");
	return 0;
}

unsigned xscale_cp14_ldc(ARMul_State * state, unsigned type, ARMword instr, ARMword data) {
	printf("xscale_cp14_ldc: ERROR isn't existed, reg15 0x%x\n", state->Reg[15]);
	SKYEYE_OUTREGS(stderr);
	skyeye_exit(-1);
}

unsigned xscale_cp14_stc(ARMul_State * state, unsigned type, ARMword instr, ARMword * data) {
	printf("xscale_cp14_stc: ERROR isn't existed, reg15 0x%x\n", state->Reg[15]);
	SKYEYE_OUTREGS(stderr);
	skyeye_exit(-1);
}

unsigned xscale_cp14_mrc(ARMul_State * state, unsigned type, ARMword instr, ARMword * data) {
//	unsigned opcode_2 = BITS (5, 7);
//	unsigned CRm = BITS (0, 3);
	unsigned reg = BITS(16, 19);
//	unsigned result;

	//printf("xscale_cp14_mrc:opcode_2 0x%x, CRm 0x%x, reg 0x%x,reg[15] 0x%x, instr %x\n",opcode_2,CRm,reg,	state->Reg[15], instr);
	...
	default:
		*data = 0;
		printf("cp14_mrc unknown cp14 regs!!!!!!\n");
		break;
	}
	return 0;
}
unsigned xscale_cp14_mcr(ARMul_State * state, unsigned type, ARMword instr, ARMword data) {
//	unsigned opcode_2 = BITS (5, 7);
//	unsigned CRm = BITS (0, 3);
	unsigned reg = BITS(16, 19);
//	unsigned result;

	//printf("xscale_cp14_mcr:opcode_2 0x%x, CRm 0x%x, reg 0x%x,reg[15] 0x%x, instr %x\n",opcode_2,CRm,reg, state->Reg[15], instr);
	...
	case PWRMODE:	// cp 14 reg 7
		...
		break;
	default:
		printf("cp14_mcr unknown cp14 regs!!!!!!\n");
		break;
	}
	return 0;
}
unsigned xscale_cp14_cdp(ARMul_State * state, unsigned type, ARMword instr) {
	printf("xscale_cp14_cdp: ERROR isn't existed, reg15 0x%x\n", state->Reg[15]);
	SKYEYE_OUTREGS(stderr);
	skyeye_exit(-1);
}
unsigned xscale_cp14_read_reg(ARMul_State * state, unsigned reg, ARMword * data) {
	printf("xscale_cp14_read_reg: ERROR isn't existed, reg15 0x%x\n", state->Reg[15]);
	SKYEYE_OUTREGS(stderr);
	skyeye_exit(-1);
}
unsigned xscale_cp14_write_reg(ARMul_State * state, unsigned reg, ARMword data) {
	printf("xscale_cp14_write_reg: ERROR isn't existed, reg15 0x%x\n", state->Reg[15]);
	SKYEYE_OUTREGS(stderr);
	skyeye_exit(-1);
}
...
//------------------------------------------------------------------
//cp15 -------------------------------------
unsigned xscale_cp15_ldc(ARMul_State * state, unsigned type, ARMword instr, ARMword data) {
	printf("xscale_cp15_ldc: ERROR isn't existed\n");
	SKYEYE_OUTREGS(stderr);
	skyeye_exit(-1);
}
unsigned xscale_cp15_stc(ARMul_State * state, unsigned type, ARMword instr, ARMword * data) {
	printf("xscale_cp15_stc: ERROR isn't existed\n");
	SKYEYE_OUTREGS(stderr);
	skyeye_exit(-1);
}
unsigned xscale_cp15_cdp(ARMul_State * state, unsigned type, ARMword instr) {
	printf("xscale_cp15_cdp: ERROR isn't existed\n");
	SKYEYE_OUTREGS(stderr);
	skyeye_exit(-1);
}
unsigned xscale_cp15_read_reg(ARMul_State * state, unsigned reg, ARMword * data) {
//chy 2003-09-03: for xsacle_cp15_cp_access_allowed
	if (reg == 15) {
		*data = state->mmu.copro_access;
		//printf("xscale_cp15_read_reg: reg 0x%x,data %x\n",reg,*data);
		return 0;
	}
	printf("xscale_cp15_read_reg: reg 0x%x, ERROR isn't existed\n", reg);
	SKYEYE_OUTREGS(stderr);
	skyeye_exit(-1);
}

//chy 2003-09-03 used by macro CP_ACCESS_ALLOWED in armemu.h
unsigned xscale_cp15_cp_access_allowed(ARMul_State * state, unsigned reg, unsigned cpnum) {
	unsigned data;

	xscale_cp15_read_reg(state, reg, &data);
	//printf("cp15_cp_access_allowed data %x, cpnum %x, result %x\n", data, cpnum, (data & 1<<cpnum));
	if (data & 1 << cpnum)
		return 1;
	else
		return 0;
...
unsigned xscale_cp15_write_reg(ARMul_State * state, unsigned reg, ARMword value) {
	switch (reg) {
	case MMU_FAULT_STATUS:
		//printf("cp15_write_reg  wrote FS        val 0x%x \n",value);
		state->mmu.fault_status = value & 0x6FF;
		break;
	case MMU_FAULT_ADDRESS:
		//printf("cp15_write_reg wrote FA         val 0x%x \n",value);
		state->mmu.fault_address = value;
		break;
	default:
		printf("xscale_cp15_write_reg: reg 0x%x R15 %x ERROR isn't existed\n", reg, state->Reg[15]);
		SKYEYE_OUTREGS(stderr)
		;
		skyeye_exit(-1);
	}
...
int xscale_cp15_init(ARMul_State * state) {
	...
	c_desc = &desc->i_cache;
	if (mmu_cache_init(I_CACHE (), c_desc->width, c_desc->way, c_desc->set, c_desc->w_mode)) {
	...
	if (mmu_cache_init(MAIN_D_CACHE (), c_desc->width, c_desc->way, c_desc->set, c_desc->w_mode)) {
		err_msg("main_d_cache init %d\n", -1);
		goto main_d_cache_init_error;
	}

	c_desc = &desc->mini_d_cache;
	if (mmu_cache_init(MINI_D_CACHE (), c_desc->width, c_desc->way, c_desc->set, c_desc->w_mode)) {
	...
	mmu_wb_exit (WB ());
#endif
	wb_init_error: mmu_cache_exit(MINI_D_CACHE ());
	mini_d_cache_init_error: mmu_cache_exit(MAIN_D_CACHE ());
	main_d_cache_init_error: mmu_tlb_exit(D_TLB ());
	d_tlb_init_error: mmu_cache_exit(I_CACHE ());
	i_cache_init_error: mmu_tlb_exit(I_TLB ());
	i_tlb_init_error: return -1;
}
...
static fault_t xscale_mmu_load_instr(ARMul_State * state, ARMword va, ARMword * instr) {
	fault_t fault;
	tlb_entry_t *tlb;
	cache_line_t *cache;
	int c;			//cache bit
...
//-----------------------------------------------------------
	...
	if (pa >= 0xa01c8000 && pa <= 0xa01c8020) {
		printf ("load_instr: pa %x, va %x,instr %x, R15 %x\n",
...
static fault_t xscale_mmu_read_byte(ARMul_State * state, ARMword virt_addr, ARMword * data) {
	//ARMword temp,offset;
	fault_t fault;
...
static fault_t xscale_mmu_read_halfword(ARMul_State * state, ARMword virt_addr, ARMword * data) {
	//ARMword temp,offset;
	fault_t fault;
...
static fault_t xscale_mmu_read_word(ARMul_State * state, ARMword virt_addr, ARMword * data) {
...
//	static unsigned chyst1 = 0, chyst2 = 0;
	...
	if (MMU_Disabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("1 xscale_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
	...
	if (datatype == ARM_WORD_TYPE) {
		if (real_va >= 0xffff0000 && real_va <= 0xffff0020) {
			...
			*data = mem_read_word (state, pa);
			chyst1++;
			printf ("**mmu_read word %d: pa %x, va %x, data %x, R15 %x\n", chyst1, pa, real_va, *data, state->Reg[15]);
			/*
...
//-------------------------------------------------
	...
	if (MMU_CacheDisabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("MMU_CacheDisabled xscale_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
		return 0;
	}
...
//chy 2003-09-02 for test only ,should commit ????
		if (real_va >= 0xffff0000 && real_va <= 0xffff0020) {
			pa = tlb_va_to_pa (tlb, va);
			chyst2++;
			printf ("**mmu_read wordk:cache %d: pa %x, va %x, data %x, R15 %x\n", chyst2, pa, real_va, *data, state->Reg[15]);
		}
//-------------------------------------------------------------------
#endif
		goto datatrans;
	...
	/*
	 if(pa >= 0xb0000000){
	 printf("xscale_mmu_read: phy address 0x%x error,reg[15] 0x%x\n",pa,state->Reg[15]);
	 return 0;
	 }
	 */
	...
	if (tlb_c_flag (tlb) && MMU_DCacheEnabled) {
		...
		*data = cache->data[va_cache_index(va, cache_t)];
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("2 xscale_mmu_read error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
		return 0;
	}

	datatrans: if (datatype == ARM_HALFWORD_TYPE) {
		temp = *data;
...
static fault_t xscale_mmu_write_byte(ARMul_State * state, ARMword virt_addr, ARMword data) {
	return xscale_mmu_write(state, virt_addr, data, ARM_BYTE_TYPE);
}

static fault_t xscale_mmu_write_halfword(ARMul_State * state, ARMword virt_addr, ARMword data) {
	return xscale_mmu_write(state, virt_addr, data, ARM_HALFWORD_TYPE);
}

static fault_t xscale_mmu_write_word(ARMul_State * state, ARMword virt_addr, ARMword data) {
...
//      static unsigned chyst1=0,chyst2=0;
	...
	if (MMU_Disabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("1 xscale_mmu_write error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
	...
	if (MMU_CacheDisabled) {
		...
		if (datatype == ARM_BYTE_TYPE)
			...
			printf("MMU_CacheDisabled xscale_mmu_write error: unknown data type %d\n", datatype);
			skyeye_exit(-1);
		}
...
//	b = tlb_b_flag (tlb);
	...
	/*
	 if(pa >= 0xb0000000){
	 printf("xscale_mmu_write phy address 0x%x error,reg[15] 0x%x\n",pa,state->Reg[15]);
	 return 0;
	 }
	 */
...
//chy 2003-09-02 for test ????
	if (datatype == ARM_WORD_TYPE) {
		if (real_va >= 0xffff0000 && real_va <= 0xffff0020) {
			printf ("**mmu_write word: pa %x, va %x, data %x, R15 %x \n", pa, real_va, data, state->Reg[15]);
		}
	}
//--------------------------------------------------------------
#endif
	...
	has_cache: index = va_cache_index(va, cache_t);
...
//chy 2003-09-02 for test ????
	if (datatype == ARM_WORD_TYPE) {
		if (real_va >= 0xffff0000 && real_va <= 0xffff0020) {
			printf ("**mmu_write word:cache: pa %x, va %x, data %x, R15 %x\n", pa, real_va, data, state->Reg[15]);
		}
	}
//-------------------------------------------------------------
#endif
...
ARMword xscale_cp15_mrc(ARMul_State * state, unsigned type, ARMword instr, ARMword * value) {
...
//	unsigned result;
	...
	/*
	 printf("xscale_cp15_mrc:opcode_2 0x%x, CRm 0x%x, reg 0x%x,reg[15] 0x%x, instr %x\n",opcode_2,CRm,reg,\
	state->Reg[15], instr);
	 */
	switch (creg) {
	case MMU_ID:		//XSCALE_CP15
		//printf("mmu_mrc read ID       \n");
		data = (opcode_2 ? state->mmu.cache_type : state->cpu->cpu_val);
		break;
	...
	default:
		data = 0;
		printf("xscale_cp15_mrc read UNKNOWN - reg %d, pc 0x%x\n", creg, state->Reg[15]);
		skyeye_exit(-1);
		break;
	}
	*value = data;
	//printf("xscale_cp15_mrc:end value  0x%x\n",data);
	return ARMul_DONE;
}
...
//chy: 2003-08-24 now, the BTB isn't simualted ....????
	...
	CRm = BITS(0, 3);
	OPC_2 = BITS(5, 7);
	//err_msg("xscale cp15_cache_ops:OPC_2 = 0x%x CRm = 0x%x, Reg15 0x%x\n", OPC_2, CRm,state->Reg[15]);
	...
	if (OPC_2 == 6 && CRm == 5) {
		...
		//printf("xscale_cp15_cache_ops:invalidate BTB CANT!!!!!!!!!!\n");
		//exit(-1);
		return;
	}

	if (OPC_2 == 5 && CRm == 2) {
		//printf("cp15_c_o: A L in D C, value %x, reg15 %x\n",value, state->Reg[15]);
		//exit(-1);
	...
	err_msg("xscale cp15_cache_ops:Unknown OPC_2 = 0x%x CRm = 0x%x, Reg15 0x%x\n", OPC_2, CRm, state->Reg[15]);
	skyeye_exit(-1);
}

static void xscale_cp15_tlb_ops(ARMul_State * state, ARMword instr, ARMword value) {
	int CRm, OPC_2;

	CRm = BITS(0, 3);
	OPC_2 = BITS(5, 7);

	//err_msg("xscale_cp15_tlb_ops:OPC_2 = 0x%x CRm = 0x%x,Reg[15] 0x%x\n", OPC_2, CRm,state->Reg[15]);
	if (OPC_2 == 0 && CRm == 0x7) {
	...
	err_msg("xscale_cp15_tlb_ops:Unknow OPC_2 = 0x%x CRm = 0x%x,Reg[15] 0x%x\n", OPC_2, CRm, state->Reg[15]);
	skyeye_exit(-1);
}

ARMword xscale_cp15_mcr(ARMul_State * state, unsigned type, ARMword instr, ARMword value) {
...
//	unsigned result;
	...
	//printf("xscale_cp15_mcr: opcode_2 0x%x, CRm 0x%x, reg ox%x, value 0x%x, reg[15] 0x%x, instr 0x%x\n",opcode_2,CRm,reg, value, state->Reg[15], instr);
...
//              printf("mmu_mcr wrote CO         val 0x%x \n",value);
	...
	default:
		printf("xscale_cp15_mcr wrote UNKNOWN - reg %d, reg15 0x%x\n", creg, state->Reg[15]);
		break;
	}
	//printf("xscale_cp15_mcr wrote val 0x%x\n", value);
	return 0;
}
//teawater add for arm2x86 2005.06.24-------------------------------------------
static int xscale_mmu_v2p_dbct(ARMul_State * state, ARMword virt_addr, ARMword * phys_addr) {
	fault_t fault;
	tlb_entry_t *tlb;
...
/*xscale mmu_ops_t*/
mmu_ops_t xscale_mmu_ops = { xscale_cp15_init, xscale_cp15_exit, xscale_mmu_read_byte, xscale_mmu_write_byte, xscale_mmu_read_halfword,
		xscale_mmu_write_halfword, xscale_mmu_read_word, xscale_mmu_write_word, xscale_mmu_load_instr, xscale_mmu_mcr, xscale_mmu_mrc,
//teawater add for arm2x86 2005.06.24-------------------------------------------
		xscale_mmu_v2p_dbct,
//AJ2D--------------------------------------------------------------------------
		};
