...
static void
at91rm92_update_int (void * state)
{
	ARMword requests = io.ipr & io.imr;
	((ARMul_State *)state)->NfiqSig = (requests & 0x00001) ? LOW : HIGH;
	((ARMul_State *)state)->NirqSig = (requests & 0xfffffffe) ? LOW : HIGH;
}
...
static void
at91rm92_io_do_cycle (void * state)
{
...
static ARMword
at91rm92_io_read_word (void * state, ARMword addr)
{
...
static ARMword
at91rm92_io_read_byte (void * state, ARMword addr)
{
	SKYEYE_DBG ("at91rm92_io_read_byte error\n");
	return at91rm92_io_read_word (state, addr);
}

static ARMword
at91rm92_io_read_halfword (void * state, ARMword addr)
{
	SKYEYE_DBG ("at91rm92_io_read_halfword error\n");
	return at91rm92_io_read_word (state, addr);
}

static void
at91rm92_io_write_word (void * state, ARMword addr, ARMword data)
{
	if ((addr >= AT91RM92_UART_BASE) &&
	    (addr < AT91RM92_UART_BASE + UART_NUM * AT91RM92_UART_SIZE)) {
...
static void
at91rm92_io_write_byte (void * state, ARMword addr, ARMword data)
{
	SKYEYE_DBG ("at91rm92_io_write_byte error\n");
	at91rm92_io_write_word (state, addr, data);
}

static void
at91rm92_io_write_halfword (void * state, ARMword addr, ARMword data)
{
	SKYEYE_DBG ("at91rm92_io_write_halfword error\n");
	at91rm92_io_write_word (state, addr, data);
}
