...
ep7312_update_int (void * state)
{
	...
	((ARMul_State *)state)->NfiqSig = (requests & 0x0001) ? LOW : HIGH;
	((ARMul_State *)state)->NirqSig = (requests & 0xfffe) ? LOW : HIGH;
}
static void
...
ARMword
ep7312_io_read_byte (void * state, ARMword addr)
{
	ARMword data = -1;
...
//	unsigned char ret;
	...
	case PEDDR:
		...
		break;
	default:
		printf ("ep7312_io_read_byte error\n");
		printf ("state->pc=%x,state->instr=%x,addr=%x\n",
				((ARMul_State*)state)->pc, ((ARMul_State*)state)->instr, addr);
		skyeye_exit (-1);
		break;
	}
	return data;
...
ARMword
ep7312_io_read_halfword (void * state, ARMword addr)
{
	printf ("ep7312_io_read_halfword error\n");
	skyeye_exit (-1);
}

ARMword
ep7312_io_read_word (void * state, ARMword addr)
{
	ARMword data = 0;
	ARMword ts_addr;
...
void
ep7312_io_write_byte (void * _state, ARMword addr, ARMword data)
{
	ARMul_State *state=(ARMul_State*)_state;
	//fprintf(stderr,"ep7312_io_write_byte,@addr is  %x",addr);
//	unsigned char offset = 0;
//	unsigned char ret;
	switch (addr - 0x80000000) {
	case PADR:
	...
	case INTMR2:
		...
		break;
	default:
		printf ("ep7312_io_write_byte error@@@@@@@\n");
...
void
ep7312_io_write_halfword (void * state, ARMword addr, ARMword data)
{
	printf ("ep7312_io_write_halfword error\n");
	skyeye_exit (-1);
}

void
ep7312_io_write_word (void * state, ARMword addr, ARMword data)
{
	ARMword tmp;
