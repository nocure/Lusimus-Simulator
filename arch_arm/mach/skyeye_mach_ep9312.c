...
ARMword
ep9312_io_read_halfword (void * state, ARMword addr)
{
...
ARMword
ep9312_io_read_word (void * state, ARMword addr)
{
	ARMword data = 0;
...
//              printf("%s(0x%08x) = 0x%08x\n", __func__, addr, data);
	...
	case SYSCON_DEVCFG:
		...
		break;
	default:
		SKYEYE_DBG ("unknown io addr, %s(0x%08x) = 0x%08x\n",
			    __func__, addr, data);
		break;
	}
	return data;
...
ARMword
ep9312_io_read_byte (void * state, ARMword addr)
{
	return ep9312_io_read_word (state, addr);
}

void
ep9312_io_write_halfword (void * state, ARMword addr, ARMword data)
{
	SKYEYE_DBG ("%s error\n", __func__);
}

void
ep9312_io_write_word (void * state, ARMword addr, ARMword data)
{
//	ARMword tmp;
	if ((addr >= EP9312_TC_BASE1) &&
	    (addr < (EP9312_TC_BASE1 + EP9312_TC_SIZE))) {
	...
	case VIC0INTENABLE:
		...
		if (data != 0x10 && data != 0x20)
			printf ("write VIC0INTENABLE=0x%x\n", data);
		extern ARMul_State * state;
		ep9312_update_int (state);
		break;
	case VIC1INTENABLE:
		io.intmr[1] = data;
//              printf("write VIC1INTENABLE=0x%x\n", data);
		extern ARMul_State * state;
		ep9312_update_int (state);
		break;
	case VIC0INTENCLEAR:
	...
	default:
		SKYEYE_DBG
			("unknown io addr, %s(0x%08x, 0x%08x), pc %x \n",
...
void
ep9312_io_write_byte (void * state, ARMword addr, ARMword data)
{
