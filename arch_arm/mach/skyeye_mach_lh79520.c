...
ARMword
lh79520_io_read_word (void * state, ARMword addr)
{
	ARMword data = 0;
...
//chy 2003-07-11: sometime has fault, but linux can continue running  !!!!????
		printf ("unknown io addr, %s(0x%08x), pc %x \n",
			__func__, addr, ((ARMul_State*)state)->Reg[15]);
...
ARMword
lh79520_io_read_byte (void * state, ARMword addr)
{
...
ARMword
lh79520_io_read_halfword (void * state, ARMword addr)
{
...
void
lh79520_io_write_word (void * state, ARMword addr, ARMword data)
{
//	ARMword tmp;
...
//                      printf("%s(%x %x)\n", __func__, addr, data);
	...
	default:
//chy 2003-07-11: sometime has fault, but linux can continue running  !!!!????
		printf ("unknown io addr, %s(0x%08x, 0x%08x), pc %x \n", __func__, addr, data, ((ARMul_State*)state)->Reg[15]);
		break;
	}
}
void
lh79520_io_write_byte (void * _state, ARMword addr, ARMword data)
{
...
void
lh79520_io_write_halfword (void * state, ARMword addr, ARMword data)
{
	printf ("%s error %x %x\n", __func__, addr, data);
	skyeye_exit (-1);
}
