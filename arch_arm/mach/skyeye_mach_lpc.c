...
//        state->NfiqSig = (~(io.vic.risr&io.vic.ier& io.vic.)) ? LOW : HIGH;
	...
	((ARMul_State*)state)->NirqSig = io.vic.isr ? LOW : HIGH;
	((ARMul_State*)state)->NfiqSig = io.vic.fsr ? LOW : HIGH;


}
static void
lpc_io_reset (void * state)
{
...
ARMword
lpc_io_read_word (void * _state, ARMword addr)
{
	/*
...
//              dataimr = unfix_int(io.intmr);
	...
	case 0xe0004008:
		data = io.timer[0].tc;
		//io.vic.risr &= ~IRQ_TC0;
		//printf("Clear TC Interrupt,tc=%x,risr=%x,\n",data,io.vic.risr);
		//lpc_update_int(state);
		break;
	case 0xe000400c:
		data = io.timer[0].pr;
...
ARMword
lpc_io_read_byte (void * state, ARMword addr)
{
...
ARMword
lpc_io_read_halfword (void * state, ARMword addr)
{
	return lpc_io_read_word (state, addr);
	//SKYEYE_OUTREGS(stderr);
	//exit(-1);
...
void
lpc_io_write_word (void * state, ARMword addr, ARMword data)
{
	/*
	 * The lpc system registers
	 */
	...
	case 0xe01fc040:
		...
		switch(io.memmap & 0x3){
			...
			case 2: /* User ram mode, vector is remapped to static ram */
				((ARMul_State*)state)->vector_remap_flag = 1;
				((ARMul_State*)state)->vector_remap_addr = 0x40000000;
				break;
			default:
...
void
lpc_io_write_byte (void * state, ARMword addr, ARMword data)
{
...
void
lpc_io_write_halfword (void * state, ARMword addr, ARMword data)
{
	lpc_io_write_word (state, addr, data);
	//SKYEYE_OUTREGS(stderr);
	//exit(-1);
