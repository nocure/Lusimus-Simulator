...
static void lpc2210_update_int(void *state)
{
	u32 irq = 0;
	int i;
	...
	//add by linxz
	//printf("RawIntr:0x%x,IntEnable:0x%x\n", io.vic.RawIntr, io.vic.IntEnable);
	...
	((ARMul_State*)state)->NirqSig = io.vic.IRQStatus ? LOW:HIGH;
	((ARMul_State*)state)->NfiqSig = io.vic.FIQStatus ? LOW:HIGH;
}
static void lpc2210_io_reset(void *state)
{
...
ARMword lpc2210_io_read_word(void *_state, ARMword addr)
{
	/*
...
//	int i;
//	ARMword dataimr = 0;
	ARMul_State *state=(ARMul_State*)_state;
...
//		dataimr = unfix_int(io.intmr);
	...
	case 0xe0004008:
		data = io.timer[0].tc;
		//io.vic.RawIntr &= ~IRQ_TC0;
		//printf("Clear TC Interrupt,tc=%x,RawIntr=%x,\n",data,io.vic.RawIntr);
		//lpc2210_update_int(state);
		break;
	case 0xe000400c:
		data = io.timer[0].pr;
	...
	default:
		...
		/*fprintf(stderr,"NumInstr %I64u, io_read_word unknown addr(0x%08x) = 0x%08x\n", state->NumInstrs, addr, data);*/
...
ARMword lpc2210_io_read_byte(void *state, ARMword addr)
{
...
ARMword lpc2210_io_read_halfword(void *state, ARMword addr)
{
		return lpc2210_io_read_word(state,addr);
		//SKYEYE_OUTREGS(stderr);
		//exit(-1);
...
void lpc2210_io_write_word(void *state, ARMword addr, ARMword data)
{
	...
	switch (addr) {
	case 0xfffff000: /* ISR */
		DBG_PRINT("can not write  ISR,it is RO,=%d\n", data);
		break;
	case 0xfffff004: /* interrupt status register */
...
//		data = unfix_int(io.intmr);
	...
	default:
		...
		/*
		fprintf(stderr,"NumInstr %I64u,io_write_word unknown addr(1x%08x) = 0x%08x\n", state->NumInstrs, addr, data);*/
		//SKYEYE_OUTREGS(stderr);
		//ARMul_Debug(state, 0, 0);
		break;
	}
...
void lpc2210_io_write_byte(void *state, ARMword addr, ARMword data)
{
...
void lpc2210_io_write_halfword(void *state, ARMword addr, ARMword data)
{
	lpc2210_io_write_word(state,addr,data);
	//SKYEYE_OUTREGS(stderr);
	//exit(-1);
