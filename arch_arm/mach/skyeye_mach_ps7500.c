...
#define PC (((ARMul_State*)state)->Reg[15])
...
//	int i;
	...
	((ARMul_State*)state)->NfiqSig = (io.fiq[FIQ] & io.fiqmask[FIQ]) ? LOW : HIGH;

	((ARMul_State*)state)->NirqSig =
...
void ps7500_kb_next(ARMul_State *state)
{
	int i;
	int p;
...
void ps7500_kb_queue(ARMul_State *state, ARMbyte c)
{
...
ARMword ps7500_io_read_byte(void *state, ARMword addr)
{
	ARMword data = 0;
	ARMword offset;
...
ARMword ps7500_io_read_halfword(void *state, ARMword addr)
{
	DBG_PRINT("ps7500_io_read_halfword error\n");
	return 0;
}
...
ARMword ps7500_io_read_word(void *state, ARMword addr)
{
	ARMword data = 0;
	ARMword data2;
	ARMword b;
...
void ps7500_io_write_byte(void *state, ARMword addr, ARMword data)
{
	ARMword offset;
...
void ps7500_io_write_halfword(void *state, ARMword addr, ARMword data)
{
	DBG_PRINT("io_write_halfword(0x%08x) <- 0x%08x\n", addr, data);
}

void ps7500_io_write_word(void *state, ARMword addr, ARMword data)
{       
	ARMword offset;
...
void ps7500_mach_init(ARMul_State *state, machine_config_t *this_mach)
{
	...
	*/
	...
	printf("%02x %02x\n", (uint32_t)state->mach_io.instr, (uint32_t)state->mach_io.net_int);
}
