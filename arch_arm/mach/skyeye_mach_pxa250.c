...
void
pxa250_io_write_byte (void * state, ARMword addr, ARMword data)
{
	return;
}

void
pxa250_io_write_halfword (void * state, ARMword addr, ARMword data)
{
	return;
}

static void pxa_set_intr (u32 interrupt)
{
...
static void
pxa250_io_write_word (void * state, ARMword addr, ARMword data)
{
	pxa_ioregnum_t ioregaddr = addr;

	//printf("pxa250_io_write_word: io addr 0x%x, RCNR 0x%x\n",ioregaddr,RCNR);
	...
	default:
		...
		//log_msg("pxa250_io_write_word: unknown addr 0x%x, reg15 0x%x \n", addr,state->Reg[15]);
		;
	};
...
ARMword
pxa250_io_read_byte (void * state, ARMword addr)
{
	return 0;
}

ARMword
pxa250_io_read_halfword (void * state, ARMword addr)
{
	return 0;
}

ARMword
pxa250_io_read_word (void * state, ARMword addr)
{
	u32 data;
//	pxa_ioregnum_t ioregaddr = addr;
	...
	default:
		data = 0;
		//chy 2003-09-03 if debug, uncommit it
		//log_msg("pxa250_io_read_word: unknown addr 0x%x, reg15 0x%x \n", addr,state->Reg[15]);
	};
