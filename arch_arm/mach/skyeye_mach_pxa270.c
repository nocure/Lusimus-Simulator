...
void
pxa270_io_write_byte (void * state, ARMword addr, ARMword data)
{
	return;
}

void
pxa270_io_write_halfword (void * state, ARMword addr, ARMword data)
{
	return;
}
...
static void
pxa270_io_write_word (void * state, ARMword addr, ARMword data)
{
...
//	mem_bank_t *mbp;
	...
	//printf("pxa270_io_write_word: io addr 0x%x, value 0x%x\n",ioregaddr,data);
	...
	case FFFCR:		//write only
		...
		//printf("write FFFCR %x, but is ffiir %x\n",pxa270_io.fffcr,pxa270_io.ffiir);
		break;
	case FFLCR:
	...
	default:
		...
		//printf("pxa270_io_write_word: unknown addr 0x%x, reg15 0x%x \n", addr,state->Reg[15]);
		;
	};
...
ARMword
pxa270_io_read_byte (void * state, ARMword addr)
{
	return 0;
}

ARMword
pxa270_io_read_halfword (void * state, ARMword addr)
{
	return 0;
}

ARMword
pxa270_io_read_word (void * state, ARMword addr)
{
	u32 data;
//	pxa_ioregnum_t ioregaddr = addr;
	...
	//printf("pxa270_io_read_word: begin io addr 0x%x \n",ioregaddr);
	...
	case FFRBR:		//chy: 2003-08-24 have some question, maybe skyeye will be locked here ????
		{
			...
			//printf("read FFRBR, but set ffiir  and ~04, now %x\n",pxa270_io.ffiir);
			pxa270_io.fflsr &= ~0x1;
	...
	default:
		data = 0;
		//chy 2003-09-03 if debug, uncommit it
		//printf("pxa270_io_read_word: unknown addr 0x%x, reg15 0x%x \n", addr,state->Reg[15]);
	};

	//printf("pxa270_io_read_word: end io addr 0x%x  data 0x%x\n",ioregaddr,data);
	return data;
};
...
pxa270_io_do_cycle (void * state)
{
	...
	if (++pxa270_io.os_scale >= OS_SCALE) {
		...
		if (count == pxa270_io.osmr3) {
			mask |= 0x8;
			if (pxa270_io.ower & 1) {
				((ARMul_State*)state)->NresetSig = LOW;
