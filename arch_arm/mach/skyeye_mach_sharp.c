...
static void
shp_io_write_byte (void * state, ARMword addr, ARMword data)
{
	...
	default:
		printf ("shp_io_write_byte %08x error, PC %x\n", addr,
			((ARMul_State*)state)->Reg[15]);
	}
	return;
}

static void
shp_io_write_halfword (void * state, ARMword addr, ARMword data)
{
	printf ("shp_io_write_halfword error\n");
	skyeye_exit (-1);
}

static void
shp_io_write_word (void * state, ARMword addr, ARMword data)
{
	shp_ioregnum_t ioregaddr = addr;
 ...
 */
	...
	default:
		printf ("shp_io_write_word: unknown addr 0x%x, reg15 0x%x \n", addr, ((ARMul_State*)state)->Reg[15]);
	}
}

static ARMword
shp_io_read_byte (void * state, ARMword addr)
{
	u32 data = 0;
	shp_ioregnum_t ioregaddr = addr;
	switch (ioregaddr) {
	...
	default:
		printf ("shp_io_read_byte %08x error, PC %x\n", addr,
				((ARMul_State*)state)->Reg[15]);
	}
//  printf("shp_io_read_byte error\n");
	return data;
}

static ARMword
shp_io_read_halfword (void * state, ARMword addr)
{
	printf ("shp_io_read_halfword error\n");
	skyeye_exit (-1);
}

static ARMword
shp_io_read_word (void * state, ARMword addr)
{
	u32 data=0;
//	shp_ioregnum_t ioregaddr = addr;
	...
	default:
		printf ("shp_io_read_word: unknown addr 0x%x, reg15 0x%x \n", addr, ((ARMul_State*)state)->Reg[15]);
	}
