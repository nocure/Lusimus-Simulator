...
static void dma_write_word(bu32 addr, bu16 value) {
	...
	case 0x2:
		...
		if (channel == BF533_MDMA_D0) {
			...
			unsigned int size = io.dma.bf533_dma_channel[BF533_MDMA_D0][X_COUNT] * abs(x_modify);
			...
			/* if 16 width is used */
			if ((((io.dma.bf533_dma_channel[BF533_MDMA_D0][DMA_CONFIG] & 0xc)
					>> 2)) == 0x1) {
				size = size / 2;
			}
			i = size;
			...
			for (; i >= 0; i--) {
				if (x_modify > 0)
					put_byte(saved_state.memory, dst++,
							get_byte(saved_state.memory, src++));
				else
					put_byte(saved_state.memory, dst--,
							get_byte(saved_state.memory, src--));
			}
...
static void bf533_set_int(bu32 irq) {
...
static void bf533_clear_int(bu32 irq_no_use) {
	int irq;
...
static void bf533_cli(bu32 * dreg) {
	*dreg = io.core_int.imask;
	//printf("cli,%x\n",*dreg);
	io.core_int.imask = 0x1f;

}
static void bf533_sti(bu32 * dreg) {
	//printf("sti,%x\n",*dreg);
	io.core_int.imask = *dreg;
}

static void bf533_io_do_cycle() {
...
static bu32 bf533_io_read_byte(void * state, bu32 addr) {
	switch (addr) {
	default:
...
static bu32 bf533_io_read_word(void * state, bu32 addr) {
	switch (addr) {
	default:
		if (addr >= DMA_IO_START_ADDR && addr < DMA_IO_END_ADDR) {
...
static bu32 bf533_io_read_long(void * state, bu32 addr) {
	switch (addr) {
	default:
		if (addr >= DMA_IO_START_ADDR && addr < DMA_IO_END_ADDR) {
...
static void bf533_io_write_byte(void * state, bu32 addr, bu32 v) {
	switch (addr) {
	default:
		if (addr >= DMA_IO_START_ADDR && addr < DMA_IO_END_ADDR) {
...
static void bf533_io_write_word(void * state, bu32 addr, bu32 v) {
	switch (addr) {
	default:
		if (addr >= DMA_IO_START_ADDR && addr < DMA_IO_END_ADDR) {
...
static void bf533_io_write_long(void * state, bu32 addr, bu32 v) {
	switch (addr) {
	default:
		if (addr >= DMA_IO_START_ADDR && addr < DMA_IO_END_ADDR) {
...
void bf533_mach_init(void * curr_state, machine_config_t * this_mach) {
