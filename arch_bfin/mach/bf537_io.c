...
static bu16 uart_read_word(bu32 addr) {
	bu16 data=0;
...
static void uart_write_word(bu32 addr, bu16 data) {
	bu32 offset = addr - UART_IO_START_ADDR;
...
static bu32 uart_read_long(bu32 addr) {
//	bu16 ret;
	IO_ERR;
	return 0;
}

static void uart_write_long(bu32 addr, bu32 value) {
	uart_write_word(addr, (value & 0xffff));
	//IO_ERR;
}

static bu8 uart_read_byte(bu32 addr) {
//	bu8 ret;
	IO_ERR;
	return 0;
}
static void uart_write_byte(bu32 addr, bu8 value) {
	IO_ERR;
}
static bu16 ebiu_read_word(bu32 addr) {
...
static bu32 ebiu_read_long(bu32 addr) {
	int offset = addr - EBIU_IO_START_ADDR;
	switch (offset) {
	case 0:
		return bf537_io.ebiu.amgctl;
...
static void ebiu_write_long(bu32 addr, bu32 v) {
	int offset = addr - EBIU_IO_START_ADDR;
	switch (offset) {
	case 0:
...
static void ebiu_write_word(bu32 addr, bu16 v) {
	int offset = addr - EBIU_IO_START_ADDR;
	switch (offset) {
	case 0:
		bf537_io.ebiu.amgctl = v;
...
static bu8 dma_read_byte(bu32 addr) {
	bu8 ret = 0;
	IO_ERR
	;
	return ret;
}
static void dma_write_byte(bu32 addr, bu8 value) {
	IO_ERR
	;
	/*
...
static bu16 dma_read_word(bu32 addr) {
	bu16 ret;
...
static void dma_write_word(bu32 addr, bu16 value) {
	int i;
	int channel = (addr & 0x3c0) >> 6;
	int offset = (addr & 0x3f) >> 2;
	...
	case 0x2:
		...
		if (channel == BF537_MDMA_D0) {
			...
			unsigned int src = bf537_io.dma.bf537_dma_channel[BF537_MDMA_S0][START_ADDR];
			unsigned int dst = bf537_io.dma.bf537_dma_channel[BF537_MDMA_D0][START_ADDR];
			short x_modify = (short)bf537_io.dma.bf537_dma_channel[BF537_MDMA_D0][X_MODIFY];
			unsigned int size = bf537_io.dma.bf537_dma_channel[BF537_MDMA_D0][X_COUNT] * abs(x_modify);

			/* if two-demisbf537_io. is used */
			if (bf537_io.dma.bf537_dma_channel[BF537_MDMA_D0][DMA_CONFIG] & 0x10) {
				size = size * bf537_io.dma.bf537_dma_channel[BF537_MDMA_D0][Y_COUNT] * bf537_io.dma.bf537_dma_channel[BF537_MDMA_D0][Y_MODIFY];
			}
			/* if 16 width is used */
			...
			for (; i >= 0; i--) {
				if (x_modify > 0)
					put_byte(saved_state.memory, dst++, get_byte(saved_state.memory, src++));
				else
					put_byte(saved_state.memory, dst--, get_byte(saved_state.memory, src--));
			}
...
static bu32 dma_read_long(bu32 addr) {
	bu32 ret;
	int channel = (addr & 0x3c0) >> 6;
	int offset = (addr & 0x3f) >> 2;
	ret = bf537_io.dma.bf537_dma_channel[channel][offset];
...
static void dma_write_long(bu32 addr, bu32 value) {
	int channel = (addr & 0x3c0) >> 6;
	int offset = (addr & 0x3f) >> 2;
	DBG_PRINT("\nIn %s channel=0x%x,offset = %x,value=%x\n",__FUNCTION__,channel,offset,value);
	bf537_io.dma.bf537_dma_channel[channel][offset] = value;
...
static void core_int_write_long(bu32 addr, bu32 v) {
...
static bu32 core_int_read_long(bu32 addr) {
	int offset = addr - CORE_INT_IO_START_ADDR;
	bu32 ret;
	//printf("read %x\n",offset);
	switch (offset) {
...
static void wd_write_word(bu32 addr, bu16 v) {
...
static bu32 deu_read_long(bu32 addr) {
...
static bu16 dpmc_read_word(bu32 addr) {
...
static void dpmc_write_word(bu32 addr, bu16 v) {
	int offset = addr - DPMC_IO_START_ADDR;
	switch (offset) {
	case 0x0:
...
static bu32 tbuf_read_long(bu32 addr) {
...
static void tbuf_write_long(bu32 addr, bu32 v) {
	int offset = addr - TBUF_IO_START_ADDR;
	switch (offset) {
	case 0:
...
static bu32 sic_read_long(bu32 addr) {
...
static void sic_write_long(bu32 addr, bu32 v) {
	int offset = addr - SIC_IO_START_ADDR;
	switch (offset) {
	case 0x24:
...
static void l1mem_write_long(bu32 addr, bu32 v) {
...
static bu32 l1mem_read_long(bu32 addr) {
	int offset = addr - L1MEM_IO_START_ADDR;
	int pos = offset >> 2;
	bu32 ret;
	switch (pos) {
...
static void l1dmem_write_long(bu32 addr, bu32 v) {
...
static void rtc_write_word(bu32 addr, bu16 v) {
...
static bu16 rtc_read_word(bu32 addr) {
	int offset = addr - RTC_IO_START_ADDR;
	bu16 ret=0;
	switch (offset) {
...
static void rtc_write_long(bu32 addr, bu32 v) {
	int offset = addr - RTC_IO_START_ADDR;
	switch (offset) {
	case 0x0:
		bf537_io.rtc.stat = v;
...
static bu32 rtc_read_long(bu32 addr) {
	int offset = addr - RTC_IO_START_ADDR;
	bu32 ret;
	switch (offset) {
	case 0x0:
...
static bu32 l1dmem_read_long(bu32 addr) {
	int offset = addr - L1DMEM_IO_START_ADDR;
	int pos = offset >> 2;
	bu32 ret;
	switch (pos) {
...
static void core_timer_write_long(bu32 addr, bu32 v) {
...
static bu32 core_timer_read_long(bu32 addr) {
	int offset = addr - CORE_TIMER_IO_START_ADDR;
	bu32 ret;
	switch (offset) {
	case 0x0:
...
static bu16 pf_read_word(bu32 addr) {
...
static void pf_write_word(bu32 addr, bu16 v) {
	int offset = addr - PF_IO_START_ADDR;
	switch (offset) {
	case 0x4:
...
static void bf537_set_int(bu32 irq) {
/*
...
static void bf537_clear_int(bu32 irq_no_use) {
	int irq;
...
static void handle_irq() {
	...
	for (irq = 0; irq < 16; irq++) {
		if (((bf537_io.core_int.ilat & (bf537_io.core_int.imask | 0x1f)) >> irq) & 0x1) {
			...
			if (bf537_io.core_int.ipend & (~((~0x0) << irq))) return;
...
static void bf537_cli(bu32 * dreg) {
	*dreg = bf537_io.core_int.imask;
	//printf("cli,%x\n",*dreg);
	bf537_io.core_int.imask = 0x1f;

}
static void bf537_sti(bu32 * dreg) {
	//printf("sti,%x\n",*dreg);
...
static void bf537_io_do_cycle(void * state) {
	...
	if ((bf537_io.sic.sic_imask & 0x800) && (bf537_io.uart.ier & 0x1) && (bf537_io.uart.lsr & 0x1)) {
...
static bu32 bf537_io_read_byte(void * p_state, bu32 addr) {
	switch (addr) {
	default:
		if (addr >= DMA_IO_START_ADDR && addr < DMA_IO_END_ADDR) {
			return dma_read_byte(addr);
		}
		else if (addr >= UART_IO_START_ADDR && addr < UART_IO_END_ADDR) {
...
static bu32 bf537_io_read_word(void * state, bu32 addr) {
	switch (addr) {
	default:
		if (addr >= DMA_IO_START_ADDR && addr < DMA_IO_END_ADDR) {
		...
		else if (addr >= DPMC_IO_START_ADDR && addr < DPMC_IO_END_ADDR) {
			return dpmc_read_word(addr);
		}
		else if (addr >= EBIU_IO_START_ADDR && addr < EBIU_IO_END_ADDR) {
...
static bu32 bf537_io_read_long(void * state, bu32 addr) {
	switch (addr) {
	default:
		if (addr >= DMA_IO_START_ADDR && addr < DMA_IO_END_ADDR) {
		...
		else if (addr >= L1MEM_IO_START_ADDR && addr < L1MEM_IO_END_ADDR) {
			return l1mem_read_long(addr);
		}
		else if (addr >= L1DMEM_IO_START_ADDR && addr < L1DMEM_IO_END_ADDR) {
			return l1dmem_read_long(addr);
		}
		else if (addr >= CORE_INT_IO_START_ADDR && addr < CORE_INT_IO_END_ADDR) {
			return core_int_read_long(addr);
		}
		else if (addr >= RTC_IO_START_ADDR && addr < RTC_IO_END_ADDR) {
			return rtc_read_long(addr);
		}
		else if (addr >= CORE_TIMER_IO_START_ADDR && addr < CORE_TIMER_IO_END_ADDR) {
			return core_timer_read_long(addr);
		}
		else if (addr >= TBUF_IO_START_ADDR && addr < TBUF_IO_END_ADDR) {
...
static void bf537_io_write_byte(void * state, bu32 addr, bu32 v) {
	switch (addr) {
	default:
		if (addr >= DMA_IO_START_ADDR && addr < DMA_IO_END_ADDR) {
...
static void bf537_io_write_word(void * state, bu32 addr, bu32 v) {
	if (addr >= DMA_IO_START_ADDR && addr < DMA_IO_END_ADDR) {
...
static void bf537_io_write_long(void * state, bu32 addr, bu32 v) {
	switch (addr) {
	default:
		if (addr >= DMA_IO_START_ADDR && addr < DMA_IO_END_ADDR) {
...
void bf537_mach_init(void * curr_state, machine_config_t * this_mach) {
