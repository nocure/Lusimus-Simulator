...
void memory_module_setup_segment(char *module_name, unsigned int *base_register, int base, int len)
{
...
// ww 2018-0820 I really don't understand this, memory_core_set_reset_values and whats in it.
#if 0
...
char Memory_Retr(unsigned int *Result, short Size, int Offset)
{
	...
	printf("retr retr failed for size=%d, offset=0x%08x, pc=0x%x\n", Size, Offset, memory_core.pc);
	skyeye_exit(1);
	return 0;
}
...
char Memory_Stor(short Size, int Offset, unsigned int Value)
{
	...
	printf("retr stor failed for size=%d, offset=0x%08x, pc=0x%x\n", Size, Offset, memory_core.pc);
	skyeye_exit(1);
	return 0;
}
