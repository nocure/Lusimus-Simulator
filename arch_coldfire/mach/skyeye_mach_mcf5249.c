...
enum {
	UMR = 0, USR, UCSR = 1, UCR = 2, URB = 3, UTB = 3, UIPCR = 4, UACR = 4, UISR = 5, UIMR = 5, UBG1 = 6, UBG2 = 6, UIVR = 12, UIP = 13, UOP1 = 14, UOP0 = 15
};
...
static char mcf5249_mbar_write(short size, int offset, unsigned int value) {
	...
	default:
		if (offset >= 0x80 && offset <= 0xAE) return cs_module_write(size, offset, value);
		if (offset >= 0x4C && offset < 0x58) {
			int index = offset - 0x4C;
			mcf5249_io.icr[index] = value;
			return 1;
		}
		if ((offset >= 0x140 && offset < 0x154) || (offset >= 0x180 && offset < 0x194)) return timer_write(size, offset, value);
		if ((offset >= 0x1C0 && offset <= 0x1FC) || (offset >= 0x200 && offset <= 0x23C)) return uart_write(size, offset, value);
...
static char mcf5249_mbar_read(unsigned int *result, short size, int offset) {
	...
	default:
		...
		if ((offset >= 0x1C0 && offset <= 0x1FC) || (offset >= 0x200 && offset <= 0x23C)) return uart_read(result, size, offset);
...
void mcf5249_io_do_cycle() {
	...
	for (i = 0; i < 2; i++) {
		...
		if (mcf5249_io.timer[i].tmr & 0x1) {
			...
			if ((mcf5249_io.timer[i].tcn == mcf5249_io.timer[i].trr) && (mcf5249_io.timer[i].tmr & 0x10)) {
				...
				if (mcf5249_io.timer[i].tmr & 0x8) mcf5249_io.timer[i].tcn = 0x0;
			}
		}
	}
	...
	if (!(mcf5249_io.ipr & 0xc)) {
		if ((mcf5249_io.uart[0].uimr & mcf5249_io.uart[0].uisr)) interrupt_assert(0xc);
	}
}
#if 0
static char cs_module_read(unsigned int *result, short size, int offset) {
