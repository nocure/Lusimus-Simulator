...
static char mcf5272_mbar_write(short size, int offset, unsigned int value) {
	...
	default:
		if (offset >= 0x20 && offset <= 0x2C) {
			...
			for (i = 31; i > 0; i = i - 4)
				if ((value >> i) & 0x1) old_value = (value & (0xf << (i - 3))) | (old_value & (~(0xf << (i - 3))));
			mcf5272_io.icr[index] = old_value;

			return 1;
		}
		if ((offset >= 0x200 && offset <= 0x272)) return timer_write(size, offset, value);
		if ((offset >= 0x100 && offset <= 0x13C) || (offset >= 0x140 && offset <= 0x17C)) return uart_write(size, offset, value);
...
static char mcf5272_mbar_read(unsigned int *result, short size, int offset) {
	...
	case MCF_MAUR:
		*result = io->ether.maur;
		break;
	default:
		if ((offset >= 0x100 && offset <= 0x13C) || (offset >= 0x140 && offset <= 0x17C)) return uart_read(result, size, offset);
		if ((offset >= 0x200 && offset < 0x270)) return timer_read(result, size, offset);
		if (offset >= 0x20 && offset <= 0x2C) {
...
void mcf5272_io_do_cycle() {
	...
	for (i = 0; i < TIMER_NUM; i++) {
		...
		if (mcf5272_io.timer[i].tmr & 0x1) {
			...
			if ((mcf5272_io.timer[i].tcn == mcf5272_io.timer[i].trr) && (mcf5272_io.timer[i].tmr & 0x10)) {
				...
				if (mcf5272_io.timer[i].tmr & 0x8) mcf5272_io.timer[i].tcn = 0x0;
			}
		}
	}
	...
	if (!(mcf5272_io.isr & UART1_IRQ)) {
		if ((mcf5272_io.uart[0].uimr & mcf5272_io.uart[0].uisr)) interrupt_assert(UART1_IRQ);
	}
}
