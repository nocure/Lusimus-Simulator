...
const char* regname[32] = { "$0", "at", "v0", "v1", "a0", "a1", "a2", "a3", "t0", "t1", "t2", "t3", "t4", "t5", "t6", "t7", "s0", "s1", "s2", "s3", "s4", "s5",
...
int decode(MIPS_State* mstate, Instr instr) {
	...
	case SPECIAL: {
		...
		case SRAV: {
			...
			UInt32 x = mstate->gpr[rt(instr)];
			int s = bits(mstate->gpr[rs(instr)], 4, 0);
			int sign_bit = 0x80000000 & x;
			x >>= s;
			UInt32 sign_bits=0;
			if (sign_bit) {
				sign_bits = 0xFFFFFFFF & (~(0xFFFFFFFF >> s));
			}
			//mstate->gpr[rd(instr)] = sign_extend_UInt32(x, 32 - s);
			mstate->gpr[rd(instr)] = sign_bits | x;
			return nothing_special;

		}
		case MOVZ: {
			if (!mstate->gpr[rt(instr)]) mstate->gpr[rd(instr)] = mstate->gpr[rs(instr)];
			return nothing_special;
		...
		case JR: {
			...
			if (mstate->pipeline == branch_delay) printf("Can't handle branch in branch delay slot\n");
		...
		case ADD: {
			...
			if (bit(x ^ y, 31) == 0 && bit(x ^ z, 31) != 0) process_integer_overflow(mstate);
		...
		case SUB: {
			...
			if ((y < 0 && z < x) || (y > 0 && z > x)) process_integer_overflow(mstate);
			mstate->gpr[rd(instr)] = z;
		...
		case TEQ: {
			// Trap If Equal
			if (mstate->gpr[rs(instr)] == mstate->gpr[rt(instr)]) fprintf(stderr, "trap happened in %s at 0x%x.\n", __FUNCTION__, mstate->pc);
			//process_reserved_instruction(mstate);
			return nothing_special;
		}
		case TNE: {
...
//			Int32 x = mstate->gpr[rt(instr)];
	...
	case PREF: {
		...
		if (print_once) printf("Warning,PREF instruction not implemented,only print once, pc=0x%x\n", mstate->pc);
		print_once = 0;
		return nothing_special;
	}
	case BEQL: {
...
//		Int32 x = mstate->gpr[rt(instr)];
	...
	case SPECIAL2: {
		...
		switch (function(instr)) {
		case MADD: {
			UInt64 temp1 = ((UInt64)mstate->hi << 32) + mstate->lo;
	...
	case LB: {
		...
		PA pa;
		if (translate_vaddr(mstate, va, data_load, &pa) != TLB_SUCC) return nothing_special;
		/*
	...
	case LH: {
		...
		PA pa;
		if (translate_vaddr(mstate, va, data_load, &pa) != TLB_SUCC) return nothing_special; //Shi yang 2006-08-10
		UInt32 x;
		UInt32 y = 0;
	...
	case LWL: {
		...
		if (!big_endian_cpu(mstate)) syscmd ^= bitsmask(1, 0);
...
//		UInt32 x;
	...
	case LWR: {
		...
		VA va = sign_extend_UInt32(offset(instr), 16) + mstate->gpr[base(instr)];
		PA pa;
		if (translate_vaddr(mstate, va, data_load, &pa)) return nothing_special; //Shi yang 2006-08-10
		UInt32 mem;
		UInt32 y = 0;
		load(mstate, round_down(va, 4), round_down(pa, 4), &y, 4);
		mem = y & (0xffffffff);
	...
	case SB: {
		...
		VA va = sign_extend_UInt32(offset(instr), 16) + mstate->gpr[base(instr)];
		PA pa;
		if (translate_vaddr(mstate, va, data_store, &pa)) return nothing_special; //Shi yang 2006-08-10
	...
	case SW: {
		...
		PA pa;
		if (translate_vaddr(mstate, va, data_store, &pa) != TLB_SUCC) return nothing_special; //Shi yang 2006-08-10
