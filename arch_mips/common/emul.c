...
int get_random(MIPS_State* mstate) //Fix me: Shi yang 2006-08-10
{
...
void leave_kernel_mode(MIPS_State* mstate) {
	mstate->cp0[SR]=copy_bits(mstate->cp0[SR], (bits(mstate->cp0[SR], 5, 2)), 3, 0);
	mstate->cp0[SR]=copy_bits(mstate->cp0[SR], (bits(mstate->cp0[SR], 3, 2) << 4), 5, 4);
 ...
 */
int asid_match(Int16 asid1, Int16 asid2) {
...
int va_match(VA va1, VA va2) {
	return !(va1 ^ va2); //Shi yang 2006-08-11 
}

// I-cache buffer operations.

int ibuf_match(PA pa, PA tag) //Shi yang 2006-08-10
{
...
int allow_xinstr(MIPS_State* mstate) {
	return mstate->mode & (kmode | xmode);
}

int branch_delay_slot(MIPS_State* mstate) //Shi yang 2006-08-10
{
...
void process_bus_error(MIPS_State* mstate, int type) {
...
void process_trap(MIPS_State* mstate) {
...
void process_breakpoint(MIPS_State* mstate) {
...
void process_fp_exception(MIPS_State* mstate) {
	process_exception(mstate, EXC_FPE, common_vector);
}

void process_interrupt(MIPS_State* mstate) {
 ...
 */
int big_endian_mem(MIPS_State* mstate) {
	return bit(mstate->cp0[Config], Config_BE);
}

int reverse_endian(MIPS_State* mstate) {
...
UInt32 swizzle_word(UInt32 x, UInt32 addr) //Fix me: Shi yang
{
	return x; //Shi yang 2006-08-18

}

UInt8 swizzle_byte(UInt32 x, UInt32 addr) {
	int offset;
	offset = (((UInt32)mstate->bigendSig * 3) ^ (addr & 3)) << 3; //Shi yang 2006-08-18
	return (x >> offset) & 0xffL;
}

UInt16 swizzle_doublebyte(UInt32 x, UInt32 addr) {
	int offset;
...
inline Int32 sign_extend_Int32(Int32 x, int n) {
	if (((Int32)-1 >> 1) < 0) {
...
inline Int64 sign_extend_Int64(Int64 x, int n) {
	if (((Int64)-1 >> 1) < 0) {
		// Host platform does arithmetic right shifts.
		Int64 y = x;
...
inline UInt64 sign_extend_UInt64(UInt64 x, int n) {
	if (((UInt64)-1 >> 1) < 0) {
		// Host platform does arithmetic right shifts.
		UInt64 y = x;
...
inline void divide_Int64(Int64 a, Int64 b) {
	if (64 <= sizeof(long)) {
		ldiv_t r = ldiv((long)(a), (long)(b));
		DivResult(r.quot, r.rem);
	}
