...
void mips_init_set(UInt32 addr, UInt8 value, int size) {

}

void mips_mem_read(UInt32 pa, UInt32 *data, int len) {
	/* if pa is located at kseg0 */
	if (pa >= 0x80000000 && pa < 0xA0000000) pa = pa & ~0x80000000;
	/* if pa is located at kseg1 */
	if (pa >= 0xA0000000 && pa < 0xC0000000) pa = pa & ~0xE0000000;
...
static void mips_init_state() {
	mstate->mode=set_bit(mstate->mode, 2);
...
void mips_trigger_irq(MIPS_State* mstate) {
	VA epc;
...
static void mips_step_once() {
	...
	/* 
	 * 1. counter value is equal to compare value
	 * 2. Compare register not equal zero (Is it right?)
	 */
	if (mstate->cp0[Compare] && (++mstate->cp0[Count] >= mstate->cp0[Compare])) {
...
static void mips_set_pc(UInt32 addr) {
	mstate->pc = addr;
}

static UInt32 mips_get_pc() {
	return mstate->pc;
}
#if 0
static void mips_write_byte(WORD addr, uint8_t v) {
	mips_mem_write_byte(addr, v);
}

static void mips_write_byte64(UInt64 addr, UInt8 data) {

}

static unsigned char mips_read_byte64(UInt64 addr) {
...
machine_config_t mips_machines[] = { { "nedved", nedved_mach_init, NULL, NULL, NULL }, { "au1100", au1100_mach_init, NULL, NULL, NULL }, { "fulong",
		fulong_mach_init, NULL, NULL, NULL }, { "gs32eb1", gs32eb1_mach_init, NULL, NULL, NULL }, { NULL, NULL, NULL, NULL, NULL }, };

static int mips_parse_cpu(const char* param[]) {
	return 1;
}

static int mips_parse_mach(machine_config_t * mach, const char* params[]) {
	int i;
	for (i = 0; i < (sizeof(mips_machines) / sizeof(machine_config_t)); i++) {
		if (!strncmp(params[0], mips_machines[i].machine_name,
		MAX_PARAM_NAME)) {
...
#if 0
static int mips_parse_mem(int num_params, const char* params[]) {
	...
	for (i = 0; i < num_params; i++) {
		...
		SKYEYE_ERR("Error: mem_bank %d has wrong parameter \"%s\".\n", num, name);
		...
		if (!strncmp("map", name, strlen(name))) {
			if (!strncmp("M", value, strlen(value))) {
				...
				SKYEYE_ERR("Error: mem_bank %d \"%s\" parameter has wrong value \"%s\"\n", num, name, value);
			}
		}
		else if (!strncmp("type", name, strlen(name))) {
			//chy 2003-09-21: process type
			if (!strncmp("R", value, strlen(value))) {
				if (mb[num].type == MEMTYPE_RAM) mb[num].type = MEMTYPE_ROM;
			...
			if (!strncmp("yes", value, strlen(value))) skyeye_config.start_address = mb[num].addr;
		}
		else {
			SKYEYE_ERR("Error: mem_bank %d has unknow parameter \"%s\".\n", num, name);
		}
	}
