...
static void gs32eb1_io_do_cycle(void * state) {
	...
	if (!(mstate->cp0[Cause] & (1 << Cause_IP4))) {
		...
		if (skyeye_uart_read(-1, &c, 1, &tv, NULL) > 0) {
			io.uart[0].thr = (int)c;
			//printf("io_do_cycle  set intcause = %x\n",io.uart[0].intcause);
		...
		if (uart_int_enable) {
			if ((io.uart[0].ier & 0x1) && (io.uart[0].iir & 0x4)) {
				io.uart[0].iir &= ~0x1;
...
static UInt32 gs32eb1_io_read_byte(void * state, UInt32 addr) {
//	UInt32 ret;
...
static UInt32 gs32eb1_io_read_halfword(void * state, UInt32 addr) {
	UInt32 ret;
	MIPS_State * mstate = (MIPS_State *)state;
	switch (addr) {
...
static UInt32 gs32eb1_io_read_word(void * state, UInt32 addr) {
...
static void gs32eb1_io_write_byte(void * state, UInt32 addr, UInt32 data) {
...
static void gs32eb1_io_write_halfword(void * state, UInt32 addr, UInt32 data) {
	MIPS_State * mstate = (MIPS_State *)state;
	switch (addr) {
	case 0x19800028:
...
static void gs32eb1_io_write_word(void * state, UInt32 addr, UInt32 data) {
	MIPS_State * curr_state = (MIPS_State*)state;
...
#if 0
static void gs32eb1_update_intr(machine_config_t *mach) {
}

static void gs32eb1_set_intr(UInt32 irq) {
	io.int_ctrl[0].req0int = irq;
}
#endif

void gs32eb1_mach_init(void * state, machine_config_t * this_mach) {
	MIPS_State * mstate = (MIPS_State*)state;
	...
	mstate->cp0[SR] = set_bit(mstate->cp0[SR], SR_CU0);
