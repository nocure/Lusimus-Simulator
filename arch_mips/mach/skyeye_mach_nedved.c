...
void mips_update_irq_flag_fast(MIPS_State* mstate) {
	...
	if ((mstate->cp0[SR] & sreg_mask) == (1 << SR_IEC)) {
		...
		if (((mstate->cp0[Cause]) & imask) && (io.intc.imr & io.intc.isr)) mstate->irq_pending = 1;

	}
}

static void nedved_io_do_cycle(void * state) {
	...
	if (io.uart.uart_char == 0) io.intc.isr |= INC_UART_RX; //UART TX IRQ
...
static UInt32 nedved_io_read_byte(void * state, UInt32 addr) {
	UInt32 ret=0;
...
static UInt32 nedved_io_read_halfword(void * state, UInt32 addr) {
	UInt32 ret=0;
...
static UInt32 nedved_io_read_word(void * state, UInt32 addr) {
	UInt32 ret=0;
...
static void nedved_io_write_byte(void * state, UInt32 addr, UInt32 data) {
...
static void nedved_io_write_halfword(void * state, UInt32 addr, UInt32 data) {
...
static void nedved_io_write_word(void * state, UInt32 addr, UInt32 data) {
...
void nedved_mach_init(void * state, machine_config_t * this_mach) {
