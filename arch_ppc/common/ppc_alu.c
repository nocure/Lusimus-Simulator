...
void ppc_opc_addmex() {
	int rD, rA, rB;
	UNUSED_VARIABLE(rB);
	PPC_OPC_TEMPL_XO(current_core->current_opc, rD, rA, rB); PPC_OPC_ASSERT(rB == 0);
	uint32 a = current_core->gpr[rA];
	uint32 ca = ((current_core->xer & XER_CA) ? 1 : 0);
 ...
 *	.442
 */
static uint32 ppc_cmp_and_mask[8] = { 0xfffffff0, 0xffffff0f, 0xfffff0ff, 0xffff0fff, 0xfff0ffff, 0xff0fffff, 0xf0ffffff, 0x0fffffff, };
...
void ppc_opc_cntlzwx() {
	int rS, rA, rB;
	UNUSED_VARIABLE(rB);
	PPC_OPC_TEMPL_X(current_core->current_opc, rS, rA, rB); PPC_OPC_ASSERT(rB==0);
	uint32 n = 0;
	uint32 x = 0x80000000;
