...
inline void ppc_fpu_add(ppc_double *res, ppc_double *a, ppc_double *b)
{
	...
	case PPC_FPR_TYPE2(ppc_fpr_norm, ppc_fpr_norm): {
		...
		if (a->s == b->s) {
			...
			res->m = a->m - b->m;
			if (!res->m) {
				if (FPSCR_RN(current_core->fpscr) == FPSCR_RN_MINF) {
					res->s |= b->s;
				} else {
					res->s &= b->s;
				}
...
inline void ppc_fpu_div(ppc_double *res, const ppc_double *a, const ppc_double *b)
{
	...
	case PPC_FPR_TYPE2(ppc_fpr_norm, ppc_fpr_norm): {
		...
		while (am && (i<56)) {
			...
			am <<= 1;
//			printf("am=%I64x, bm=%I64x, rm=%I64x\n", am, bm, res.m);
			i++;
		}
		res->m <<= 57-i;
		if (res->m & (1ULL << 56)) {
...
//		printf("final: am=%I64x, bm=%I64x, rm=%I64x\n", am, bm, res.m);
		break;
	}
	case PPC_FPR_TYPE2(ppc_fpr_NaN, ppc_fpr_NaN):
		res->e = a->e;
...
inline void ppc_fpu_sqrt(ppc_double *D, const ppc_double *B)
{
	...
	case ppc_fpr_norm:
		...
		*D = *B;
		...
		for (i=0; i<6; i++) {
			ppc_double D_old = *D;
			ppc_double B_div_D_old={0,0,0,0};
...
void ppc_opc_faddx()
{
	...
	ppc_fpu_add(&D, &A, &B);
	current_core->fpscr |= ppc_fpu_pack_double(&D, &(current_core->fpr[frD]));
	if (current_core->current_opc & PPC_OPC_Rc) {
		// update cr1 flags
		PPC_FPU_ERR("fadd.\n");
	}
...
void ppc_opc_faddsx()
{
	...
	ppc_fpu_add(&D, &A, &B);
	current_core->fpscr |= ppc_fpu_pack_double_as_single(&D, &(current_core->fpr[frD]));
	if (current_core->current_opc & PPC_OPC_Rc) {
		// update cr1 flags
		PPC_FPU_ERR("fadds.\n");
	}
...
void ppc_opc_frspx()
{
	...
	ppc_double B;
	ppc_fpu_unpack_double(&B, current_core->fpr[frB]);
	current_core->fpscr |= ppc_fpu_pack_double_as_single(&B, &(current_core->fpr[frD]));
	if (current_core->current_opc & PPC_OPC_Rc) {
		// update cr1 flags
		PPC_FPU_ERR("frsp.\n");
	}
