...
//	int pid_match = 0;
	...
	while (i++ < (L2_TLB0_SIZE + L2_TLB1_SIZE)) {
		...
		if (entry->tid != 0) {
			...
			if (current_core->mmu.pid[0] != entry->tid) continue;
		...
		if (i > (L2_TLB0_SIZE - 1)) {
			...
			mask = ~(1024 * 4 - 0x1);
		if (entry->size != 0xb) {
			if ((addr & mask) != ((entry->epn << 12) & mask)) continue;
			/* check rwx bit */
	...
	if (flags == PPC_MMU_CODE) {
		...
		if (ppc_exception(core, DATA_TLB, flags, addr)) return PPC_MMU_EXC;
	}
...
static byte lvsl_helper[] = { 0x1F, 0x1E, 0x1D, 0x1C, 0x1B, 0x1A, 0x19, 0x18, 0x17, 0x16, 0x15, 0x14, 0x13, 0x12, 0x11, 0x10, 0x0F, 0x0E, 0x0D, 0x0C, 0x0B,
...
void ppc_opc_tlbivax() {
	...
	if (addr & 0x4) {
		...
		if (((addr >> 3) & 0x3) > 1) return;
		if ((addr >> 3) & 0x3) {
			for (j = 0; j < L2_TLB1_SIZE; j++)
				if (!current_core->mmu.l2_tlb1_vsp[j].iprot) current_core->mmu.l2_tlb1_vsp[j].v = 0;

		}
		else {
			for (i = 0; i < L2_TLB0_SIZE; i++)
				if (!current_core->mmu.l2_tlb0_4k[i].iprot) current_core->mmu.l2_tlb0_4k[i].v = 0;
		}
		return;
	}
	/* walk over tlb0 and tlb1 to find the entry */
