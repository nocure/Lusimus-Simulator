 ...
 *       Filename:  i_addcc.c
 *
 *    Description:  Implementation of the ADDCC instruction where the carry bit
...
31-30  29-25   24-19     18-14    13        12-5          4-0
+----+-------+--------+------------------------------------------+
| 10 | 00000 | 000010 |   rs1   | 1/0 |      simm12/rs2          |
+----+-------+--------+------------------------------------------+
...
#define ADDCC_CYCLES    1
#define ADDCC_CODE_MASK 0x80800000
#define PRIVILEDGE  0
...
static int execute(void *state)
{
    ...
    if( imm < 0 )
    {
        ...
        print_inst_RS_RS_RD("addcc", rs1, rs2, rd);
//        DBG("addcc %s, %s, %s\n", RNAME(rs1), RNAME(rs2), RNAME(rd));
    ...
    else
    {
        ...
        print_inst_RS_IM13_RD("addcc", rs1, sign_ext13(imm), rd);
//        DBG("addcc %s, %d, %s\n", RNAME(rs1), sign_ext13(imm), RNAME(rd));
    }
...
static int disassemble(uint32 instr, void *state)
{
    ...
    if( (instr & ADDCC_CODE_MASK) && (op == OP) && (op3 == OP3) )
    {
