 ...
 * =====================================================================================
 */
static uint32 leon2_uart_write(void *opaque, uint32 addr, uint32 v) {
	int reg;
...
static uint32 leon2_uart_read(void *opaque, uint32 addr) {
	...
	if (reg == UART_DATA_REGISTER) leon2_uart.regs.status.data_ready = 0;
...
//    bzero( (void*)&leon2_uart.regs, sizeof(struct regs));
	memset((void*)&leon2_uart.regs, 0, sizeof(struct regs));
	...
	/*  Register the timer  */
	leon2_register_io_memory(address, UART_NREGS * 4, leon2_uart_write, leon2_uart_read, NULL);
}
