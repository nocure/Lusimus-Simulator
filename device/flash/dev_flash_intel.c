...
//	ARMul_State *state = (ARMul_State *) mc->state;
	...
	//CHECK_ADDR_RANGE(addr);
        ...
        //fprintf(stderr,"flash write begin: addr %x  data %x\n", addr,data);
	switch (io->wsm_mode) {
	case WSM_READY:
	...
	case WSM_WRITE_BUFFER:
		if (io->pb_count == 0) {	/* step 1: get count */
			...
			//fprintf(stderr,"flash write step 1: count %x  addr %x\n",io->pb_count, addr);
		...
		//fprintf(stderr,"flash write step 2: load data: start_addr %x, count %x\n",io->pb_start, io->pb_loaded);
			...
			if (ISLOCKED (io->progbuf_latch_addr)) {
				...
				io->program_setlb_error =
					io->program_volt_error = 1;
		              ...
		              //fprintf(stderr,"flash write step 3: confirm: \n");
				for (j = WORD_ADDR (io->pb_start);
					...
					*/
					...
					//printf("In %s, mem_read 0x%x, mem_write 0x%x\n", __FUNCTION__, tmp, tmp & io->pb_buf[j & INTEL_WRITEBUFFER_MASK]);
				}
			}
			io->wsm_mode = WSM_READY;
			io->progbuf_busy = 0;
