...
static int SkyEyeLCD_Initalize(SkyEyeLCD *self)
{
	...
	snprintf(mode, 100, "%dx%dx%d %s", self->fWidth, self->fHeight, self->fDepth, "LCD");
...
//			bzero(lcd->fPrivateADDR, sizePrivateADDR);
			memset(lcd->fPrivateADDR, 0, sizePrivateADDR);
...
// ww 2018-0816 modified: no idea whoever wrote this wants to achieve.
//	*buf_addr = ++(*buf_addr);
	(*buf_addr)++;
}
...
//			printf ("lcd_line_offset:%d\n", (LONG)lcd->fDevice->lcd_line_offset);
			bitsInfo->bmiHeader.biBitCount = 32;
			bitsInfo->bmiHeader.biCompression = BI_RGB;
			bitsInfo->bmiHeader.biSizeImage = 0;
			break;
