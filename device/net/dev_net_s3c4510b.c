...
//	fault_t fault;
	...
	int i;
//	fault =
	mmu_read_word (state, io->bdmatxptr, &ptr);
	/*
	   if( fault ) {
	   *addr = io->bdmatxptr;
	   return fault;
	...
	ptr &= ~BDMA_owner;
//	fault =
	mmu_read_word (state, io->bdmatxptr + 8, &len);
	/*
	   if( fault ) {
	   *addr = io->bdmatxptr + 8;
	   return fault; 
	...
	for (i = 0; i < len; i++) {
//		fault =
		mmu_read_byte (state, ptr + i, (ARMword*)(io->mac_buf + i));
		/*
		   if( fault ) {
		   *addr = ptr + i;
		   return fault;
	...
	status = len | (Comp << 16);
//	fault =
	mmu_write_word (state, io->bdmatxptr + 8, status);
...
//	fault =
	mmu_write_word (state, io->bdmatxptr, ptr);
	/*
	   if( fault ) {
	   *addr = io->bdmatxptr;
	   return fault;
	...
	//get next desc
//	fault =
	mmu_read_word (state, io->bdmatxptr + 12, &io->bdmatxptr);
	/*
	   if( fault ) {
...
//	int n
	...
	if (packet_len <= 0)
		return;
//	fault =
	mmu_read_word (state, io->bdmarxptr, &ptr);
...
//		fault =
	...
	//printf("status_len:%x\n",status_len); 
//	fault =
	mmu_write_word (state, io->bdmarxptr + 8, status_len);
	/*
	   if(fault) {
	   *addr = io->bdmarxptr+8;
	   return fault;
	...
	//set owner bit of desc to CPU
//	fault =
	mmu_write_word (state, io->bdmarxptr, ptr);
	/*
	   if(fault) {
	   *addr = io->bdmarxptr;
	   return fault;
	...
	//get next desc
//	fault =
	mmu_read_word (state, io->bdmarxptr + 12, &io->bdmarxptr);
	/*
	   if(fault) {
