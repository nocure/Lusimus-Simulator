...
int skyeye_uart_setup(struct uart_option *uart_opt)
{
	...
	switch (uart_dev->mod) {
		...
		case UART_SIM_TELNET:
			uart_dev->uart_open = uart_telnet_open;
			uart_dev->uart_close = uart_telnet_close;
			uart_dev->uart_read = uart_telnet_read;
			uart_dev->uart_write = uart_telnet_write;
			ret = 0;
			break;
