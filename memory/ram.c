...
mem_read_byte (uint32_t addr)
{
	...
	data = global_memory.rom[global_mbp - memmap->mem_banks]
	                        [(addr - global_mbp->addr) >> 2];
...
mem_write_byte (uint32_t addr, uint32_t data)
{
	...
	temp = &global_memory.rom[global_mbp - memmap->mem_banks]
	                         [(addr - global_mbp-> addr) >> 2];
	offset = (((uint32_t) arch_instance->big_endian * 3) ^ (addr & 3)) << 3;	/* bit offset into the word */
...
#endif
	temp = (unsigned long*)&global_memory.rom[global_mbp -
			       memmap->mem_banks][(addr -
							    global_mbp->
							    addr) >> 2];
	offset = (((uint32_t) arch_instance->big_endian * 2) ^ (addr & 2)) << 3;	/* bit offset into the word */
...
//	mem_state_t * mem = &global_memory;
	...
	for (i = 0; i < num; i++) {
		...
		if (mb[bank].type != MEMTYPE_IO) {
			...
			if (!global_memory.rom[bank]) {
				fprintf (stderr,
					 "mem_reset: Error allocating mem for bank number %d.\n",
					 bank);
				skyeye_exit (-1);
			}
// wsw 2009-0915 added { clear allocated memory
			memset(global_memory.rom[bank],0,mb[bank].len);
// wsw 2009-0915 added }
			/*ywc 2005-04-01 */
#ifdef DBCT
			if (!skyeye_config.no_dbct) {
...
int mem_read(short size, uint32_t offset, uint32_t * value){
//	void * state;
	switch(size){
		case 8:
			// ww 2009-018 fixed: remove uint8_t
			*/*(uint8_t *)*/value = (uint8_t)mem_read_byte (offset);
			break;
		case 16:
			// ww 2009-018 fixed: remove uint16_t
...
int mem_write(short size, uint32_t offset, uint32_t value){
	switch(size){
		case 8:
			// ww 2009-018 fixed: added &0xff to make sure a BYTE
			mem_write_byte (offset, value&0xff);
			break;
		case 16:
			// ww 2009-018 fixed: added &0xffff to make sure a WORD
			mem_write_halfword(offset, value&0xffff);
			break;
		case 32:
...
unsigned char * get_dma_addr(unsigned long guest_addr){
        ...
        mem_config_t * memmap = get_global_memmap();
        host_addr = (unsigned char*)&global_memory.rom[global_mbp -
...
int warn_write(short size, uint32_t offset, uint32_t value){
