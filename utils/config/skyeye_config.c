...
// ww 2014-0113 moved static skyeye_option_t skyeye_options[] here, it's only file refer to the array.
static skyeye_option_t skyeye_options[] = {
	...
	{ "rwio", do_rwio_option, 0, 1 },		// ww 2014-0113 added
	{ "sefsio", do_sefsio_option, 0, 1 },	// ww 2014-0113 added
 ...
 * */
static int parse_line_formatted(int num_params, const char *params[]) {
	int i;
	...
	for (i = 0; i < len; i++, sop++) {
		if (!strncmp(sop->name, params[0], MAX_OPTION_NAME)) {
			...
			if (sop->do_num > sop->max_do_num) {
				/**/fprintf(stderr, "\"%s\" option has exceeded max number %d!\n", params[0], sop->max_do_num);
				return -1;
			}
			else if ((retval = sop->do_option(sop, num_params - 1, &params[1])) < 0) {
...
#define MAX_PARAMS_LEN 40
	...
	while ((ptr) && (!comment)) {
		...
		for (i = 0; i < strlen(ptr); i++) {
			if (ptr[i] == '"')
				...
				if (!isspace(ptr[i]) || inquotes) string[string_i++] = ptr[i];
			}
		}
		string[string_i] = '\0';
		if (string_i == 0) break;
		...
		if (num_params < MAX_PARAMS_LEN) {
			...
			if (!params[num_params]) {
				printf("parse_line_unformatted: malloc params[%d] error\n", num_params);
				skyeye_exit(-1);
			}
	 ...
	 */
	retval = parse_line_formatted(num_params, (const char**)&params[0]);
	for (i = 0; i < MAX_PARAMS_LEN; i++) {
		if (params[i] != NULL) {
			free(params[i]);
			params[i] = NULL;
...
int skyeye_read_config() {
	...
	if ((config_fd = fopen(skyeye_config.config_file, "r"))) {
		...
		if (retval < 0) {
			fprintf(stderr, "config file %s have errors!\n", skyeye_config.config_file);
			return -1;
		}
	}
	else {
		fprintf(stderr, "*ERROR* Failed to open system config file \"%s\"\n", skyeye_config.config_file);
		return -1;
	}
//chy 2003-08-20
#if 0
	...
	fprintf (stderr, "log file is /tmp/skyeyelog\n");
	else {
		fprintf (stderr,
