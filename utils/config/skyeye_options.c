...
//	char *default_arch = "arm";
	...
	SKYEYE_ERR
		("Error: Unknowm architecture name \"%s\".\n",
		 params[0]);
	return -1;
}
	       ...
	       const char *params[])
{
	...
	for (i = 0; i < num_params; i++) {
		...
		if (!strncmp ("mac", name, strlen (name))) {
			...
			sscanf (value, "%d.%d.%d.%d", (int*)&hip[0], (int*)&hip[1],	(int*)&hip[2], (int*)&hip[3]);
		...
		const char *params[])
{
	...
	for (i = 0; i < num_params; i++)
	{
		...
		if (!strncmp("mod", name, strlen(name))) {
			...
			else if (!strncmp ("telnet", value, strlen (value)))
				uart_opt.mod = UART_SIM_TELNET;
		}
...
// ww 2014-0113 added
int do_rwio_option (skyeye_option_t * this_option, int num_params, const char *params[]) {
	char name[MAX_PARAM_NAME], value[MAX_PARAM_NAME];
	struct rwio_option rwio_opt;
	int i;

	for (i = 0; i < num_params; i++) {
		if (split_param (params[i], name, value) < 0)
			SKYEYE_ERR
				("Error: rwio has wrong parameter \"%s\".\n",
				 name);

		/* TODO */
	}

	setup_device_option (this_option->name, (void*)&rwio_opt,
			     num_params, params);
...
// ww 2015-1029 added
int do_sefsio_option (skyeye_option_t * this_option, int num_params, const char *params[]) {
	char name[MAX_PARAM_NAME], value[MAX_PARAM_NAME];
	struct sefsio_option sefsio_opt;
	int i;

	for (i = 0; i < num_params; i++) {
		if (split_param (params[i], name, value) < 0)
			SKYEYE_ERR
				("Error: sefsio has wrong parameter \"%s\".\n",
				 name);

		if (!strncmp ("root", name, strlen (name))) {
			sefsio_opt.root=strdup(value);
		}
	}

	setup_device_option (this_option->name, (void*)&sefsio_opt,
			     num_params, params);
		 ...
		 const char *params[])
{
	int i;
	char name[MAX_PARAM_NAME], value[MAX_PARAM_NAME];
	extern unsigned int load_base;
	extern unsigned int load_mask;
	for (i = 0; i < num_params; i++) {
		if (split_param (params[i], name, value) < 0)
			SKYEYE_ERR
				("Error: sound has wrong parameter \"%s\".\n",
	...
	for (i = 0; i < num_params; i++) {
		...
		if (!strncmp ("state", name, strlen (name))) {
			if (!strncmp ("on", value, strlen (value))) {
				...
				SKYEYE_INFO
					("code coverage info: enabled!\n");
			}
			else {
				skyeye_config.code_cov.prof_on = 0;
				SKYEYE_INFO
					("code coverage info: disabled!\n");
			}
		}
		else if (!strncmp ("filename", name, strlen (name))) {
			...
			SKYEYE_INFO ("log_info: log start address %u\n",
				     start);
		}
		else if (!strncmp ("end", name, strlen (name))) {
			end = strtoul (value, NULL, 0);
			skyeye_config.code_cov.end = end;
			SKYEYE_INFO ("log_info: log end address %u\n", end);
		}
	}
	return 0;
}
