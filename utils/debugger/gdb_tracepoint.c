...
extern void fetch_inferior_registers (int regno, char *memory);
...
void trace_fetch_registers(int regno, char *memory)
{
...
int trace_read (unsigned int addr, char *buffer, int size)
{
    collect_record *current_record;
    if(skyeye_ice.selected_fb!=NULL)
    { 
...
int select_frame_buffer_by_fn (unsigned int *tracepoint_number,unsigned int frame_number)
{
...
int select_frame_buffer_by_tpn (unsigned int tracepoint_number, unsigned int *frame_number)
{
    frame_buffer * current_fb;
    if (skyeye_ice.selected_fb==NULL)
    { 
