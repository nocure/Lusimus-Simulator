...
//	struct protoent *protoent;
...
#else
	if ((tmp_desc = socket(PF_INET, SOCK_STREAM, IPPROTO_TCP)) == INVALID_SOCKET) wsa_perror("[GDB]Can't open socket");
#endif
	...
	if (bind(tmp_desc, (struct sockaddr *)&sockaddr, sizeof(sockaddr)) || listen(tmp_desc, 1)) wsa_perror("[GDB]Can't bind address");

	tmp = sizeof(sockaddr);
	remote_desc = accept(tmp_desc, (struct sockaddr *)&sockaddr, &tmp);
	if (remote_desc == -1) wsa_perror("[GDB]Accept failed");
	...
	tmp = 1;
	setsockopt(remote_desc, 6 /* PROTO_TCP */, TCP_NODELAY, (char *)&tmp, sizeof(tmp));
#endif
...
#endif /* FASYNC */
#endif //chy 20050729-------------------
	printf("Remote debugging using port %d\n", port);
}
...
static int fromhex(int a) {
	if (a >= '0' && a <= '9')
		...
		fprintf(stderr, "Reply contains invalid hex digit");
	return 0;
}
	...
	*p = '\0';
	...
	do {
		...
		if (Write (remote_desc, buf2, p - buf2) != p - buf2) {
			wsa_perror("[GDB]putpkt(write)");
			return -1;
		}

		if (remote_debug) DBG_RDI ("putpkt (\"%s\"); [looking for ack]\n", buf2);
		cc = Read(remote_desc, buf3, 1);
		if (remote_debug) DBG_RDI ("[received '%c' (0x%x)]\n", buf3[0], buf3[0]);
		if (cc <= 0) {
			if (cc == 0)
				fprintf(stderr, "putpkt(read): Got EOF\n");
			else
				wsa_perror("[GDB]putpkt(read)");
...
input_interrupt ()
{
	...
	DBG_RDI ("debugger: input_interrupt: get a ctrl-c\n");
	skyeye_exit (-1);
...
int getpkt(char *buf) {
	...
	while (1) {
		...
		while (1) {
			c = readchar();
			if (c == '$') break;
			if (remote_debug) DBG_RDI ("[getpkt: discarding char '%c']\n",
					c);
			if (c < 0) return -1;
		}
		...
		*bp = 0;
		...
		if (csum == (c1 << 4) + c2) break;

		fprintf(stderr, "Bad checksum, sentsum=0x%x, csum=0x%x, buf=%s\n", (c1 << 4) + c2, csum, buf);
		Write(remote_desc, "-", 1);
	}

	if (remote_debug) DBG_RDI ("getpkt (\"%s\");  [sending ack] \n", buf);

	Write(remote_desc, "+", 1);

	if (remote_debug) DBG_RDI ("[sent ack]\n");
	return bp - buf;
}
...
static void convert_ascii_to_int(char *from, char *to, unsigned int n) {
	int nib1, nib2;
	while (n--) {
		nib1 = fromhex(*from++);
		nib2 = fromhex(*from++);
...
outreg(int regno, char *buf) {
	...
	*buf++ = tohex(regno & 0xf);
	*buf++ = ':';
	convert_int_to_ascii(&registers[current_reg_type->register_byte(regno)], buf, regsize);
	buf += 2 * regsize;
	*buf++ = ';';
...
void fetch_inferior_registers(int regno, char *memory) {
	if (regno == -1 || regno == 0)
...
static void store_inferior_registers(int regno, char *memory) {
	if (regno == -1 || regno == 0)
		for (regno = 0; regno < current_reg_type->num_regs; regno++)
...
int sim_debug(int port) {
	...
	printf("Entering GDB server mode, listening on TCP port %d\n", port);

	registers = (char *)malloc(current_reg_type->register_bytes);
...
// ww: added to reset cpu when GDB server connected
		arch_instance->reset();
...
//			unsigned char sig;
			...
			case 'g':
				...
				break;
			case 'G':
				convert_ascii_to_int(&own_buf[1], registers, current_reg_type->register_bytes);
			...
			case 'M':
				...
				//printf("debugger M  addr 0x%x, word 0x%x, len 0x%x\n", mem_addr, *((unsigned int *)mem_buf), len);
			...
			case 'q':
				/*
				 * ww 2018-0806 modified
				 * newer versions of gdb (arm-none-eabi-gdb) sends qTfV, qTsV command to query tracepoints.
				 * since we don't support any tracepoint functions, so just return null back to gdb
				 * to avoid probelm.
				 *
				 */
#if 0
			...
			case 'z': {
				...
				//printf("gdbserver z, type %d, addr %x, len %x\n",type, addr,len);
				if (type == 0 || type == 1) {
					if (sim_ice_breakpoint_remove(addr) < 0) goto remove_breakpoint_error;
					write_ok(own_buf);
				}
				else {
					remove_breakpoint_error: write_enn(own_buf);
				}
			}
				break;
			case 'Z': {
				...
				//printf("gdbserver Z, type %d, addr %x, len %x\n",type, addr,len);
				if (type == 0 || type == 1) {
					if (sim_ice_breakpoint_insert(addr) < 0) goto insert_breakpoint_error;
					write_ok(own_buf);
				}
				else {
					insert_breakpoint_error: write_enn(own_buf);
				}
			}
				break;
#if 0				// chy 2005-07-30
			...
			if (status == 'W') {
// ww 2018-0816 modified: for some reason sig is never set. so don't use it
...
//				fprintf (stderr,"\nChild terminated with signal = 0x%x\n",sig);
			...
			if (status == 'W' || status == 'X') {
				if (extended_protocol) {
					//chy 2005-07-30
					fprintf(stderr, "CHY not Killing inferior\n");
					//kill_inferior ();
					write_ok(own_buf);
		...
		if (extended_protocol) {
			...
			fprintf(stderr, "GDB server connection terminated.\n");
			remote_close();
		}
	}
}
...
int remote_interrupt() {
	...
	if (select(remote_desc + 1, &rfds, NULL, NULL, &tv) == 1) {
		...
		if (n > 0) DBG_RDI("Got async char");
		if (buf[0] == '\003') DBG_RDI(":Ctrl+C, interrupted.\n");
		return 1;
	}
	return 0;
}
