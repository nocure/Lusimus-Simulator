...
static int mips_store_register(int rn, char * memory){
	...
	if(rn >= 0 && rn < 32)
		...
		mstate->fpr[rn-38] = v; /*mstate->fpr[rn] = v;*/	// ww 2018-0815 modified: the original code exceed fp reg array
	else if(rn == 70 || rn == 71 || rn == 72) /* fsr, fir, fp */
		; /* do nothing */
	return 0;
}
static int mips_fetch_register(int rn, char * memory){
	uint32_t v=0;
	if(rn >= 0 && rn < 32)
		v = mstate->gpr[rn];
	else if(rn == 32)/* SR register of CP0 */
		...
		v = mstate->fpr[rn-38]; /*v = mstate->fpr[rn];*/		// ww 2018-0815 modified: the original code exceed fp reg array
	else if(rn == 70 || rn == 71 || rn == 72) /* fsr, fir, fp */
		/* do nothing */
