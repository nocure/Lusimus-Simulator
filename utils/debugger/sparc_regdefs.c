...
#include "../arch_sparc/common/sparc.h"
#include "../arch_sparc/common/traps.h"
#include "../arch_sparc/common/iu.h"
 ...
 * =====================================================================================
 */
static int sparc_store_register(int rn, char * memory)
{
    uint32 val;
    ...
    if( rn >= G0 && rn<= I7 )
    	...
    	FPREG(rn-F0) = val; /*FPREG(rn) = val;*/	// ww 2018-0815 modified: the original code exceed fp reg array
    else if( rn == Y )
        YREG = val;
    else if( rn == PSR )
        PSRREG = val;
 ...
 * =====================================================================================
 */
static int sparc_fetch_register(int rn, char * memory)
{
    uint32 val;
    ...
    if( rn >= G0 && rn<= I7 )
    	...
    	val = FPREG(rn-F0); /*val = FPREG(rn);*/ // ww 2018-0815 modified: the original code exceed fp reg array
    else if( rn == Y )
        val = YREG;
    else if( rn == PSR )
        val = PSRREG;
...
void init_sparc_register_defs(void)
{
	...
	sparc_reg_defs.endian_flag = HIGH;
